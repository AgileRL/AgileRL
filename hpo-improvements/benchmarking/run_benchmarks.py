#!/usr/bin/env python3
"""Benchmarking suite runner for HPO algorithm comparison.
...
are logged to the ``hpo-improvements`` project with descriptive run names.

Example usage::

    # Run all benchmarks on GPU with the baseline HPO algorithm
    uv run python hpo-improvements/benchmarking/run_benchmarks.py \\
...
WANDB_PROJECT = "hpo-improvements"
...
def build_command(
    ...
    *,
    ...
    cmd = [
        sys.executable, "-m", "agilerl.train",
        "-m", str(config_path),
        "--wb",
...
def _benchmark_table() -> str:
    ...
    for bid, info in sorted(BENCHMARKS.items()):
        lines.append(f"  {bid:>2}  {info['algo']:<8} {info['complexity']:<8} {info['config']}")
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"\n{'=' * 60}")
    print(f"  HPO Benchmarking Suite")
    ...
    for r in results:
        ...
        print(f"  [{r['id']:>2}] {binfo['algo']:<8} {binfo['complexity']:<8} => {r['status']}{elapsed}")
    print()
