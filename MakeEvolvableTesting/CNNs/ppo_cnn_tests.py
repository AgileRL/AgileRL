...
if __name__ == "__main__":
    ...
    NET_CONFIG = {
        ...
        "h_size": [64],  # Actor hidden size
        'c_size': [32], # CNN channel size
        'k_size': [3],   # CNN kernel size
    ...
    state_dim = np.moveaxis(np.zeros(state_dim[0]), [2], [0]).shape
    ...
    if wb:
        wandb.init(
            ...
            project="EvoWrappersConnect4",
            name="{}-EvoHPO-{}-RandomOpposition-CNN-{}".format(
    ...
    for idx_epi in pbar:
        for agent in pop:  # Loop through population
            for episode in range(episodes_per_epoch):
                ...
                p0_log_probs, p0_log_probs = [], []
                p0_rewards, p0_rewards = [], []
                p0_dones, p0_dones = [], []
                p0_values, p0_values = [], []
                ...
                for idx_step in range(max_steps):
                    ...
                    p0_state = np.expand_dims(np.moveaxis(p0_state, [2], [0]), 0)
                    if opponent_first:
                        p0_action = env.action_space("player_0").sample(p0_action_mask)
                    else:
                        p0_action = agent.getAction(p0_state, epsilon, p0_action_mask)[
                            0
                    ...
                    p0_next_state = np.expand_dims(np.moveaxis(p0_next_state, [2], [0]), 0)
                    ...
                    if done or truncation:
                        ...
                        memory.save2memoryVectEnvs(
                            np.concatenate((p0_state, p1_state)),
                            [p0_action, p1_action],
                            [reward, -reward],
                            np.concatenate((p0_next_state, p1_next_state)),
                            [done, done],
                        )
                    else:
                        p0_reward = reward
                        ...
                        p1_state = np.expand_dims(np.moveaxis(p1_state, [2], [0]), 0)
                        if not opponent_first:
                            p1_action = env.action_space("player_1").sample(
                                p1_action_mask
                            )
                        else:
                            p1_action = agent.getAction(p1_state, epsilon, p1_action_mask)[
                                0
                            ]  # Get next action from agent
                        ...
                        p1_next_state = np.expand_dims(np.moveaxis(p1_next_state, [2], [0]), 0)
                        ...
                        for idx_step in range(max_steps):
                            ...
                            if player > 0:
                                if not opponent_first:
                                    ...
                                    state[:, :, [0, 1]] = state[:, :, [1, 0]]
                                    state = np.expand_dims(np.moveaxis(state, [2], [0]), 0)
    ...
    path = "./models/DQN"
    filename = "DQN_random_opp_trained_agent_cnn.pt"
