...
#### Type checking
AgileRL is type-checked with [`ty`](https://docs.astral.sh/ty/) and ships `py.typed` markers, so the public API's annotations are part of the released package. The configuration lives in `pyproject.toml` under `[tool.ty]`: modules are checked strictly unless they appear in the documented legacy override list, which shrinks over time as modules are migrated. The `Type checks` GitHub workflow blocks PRs on new type errors. If you touch a module in the legacy list, consider removing it from the list and fixing what surfaces — but keep that migration to its own commit.
