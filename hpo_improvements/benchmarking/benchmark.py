...
def fetch_and_plot(
    *,
    ...
    # read from the W&B history (degrade to a placeholder when missing). The
    # mutation-efficacy and population-dynamics figures are deliberately not
    # produced -- they derived from the per-generation mutation-history CSV, which
    # cannot faithfully represent recombination (crossover) or migration (MF-PBT),
    # so neither the CSV nor those figures are emitted in this benchmark.
