"""Train DQN on EnvPool Google Research Football run_to_score.
    ...
    *,
    ...
    try:
        ...
        msg = (
            ...
            "`uv add envpool` (and ensure GFootball support is available)."
        )
        raise ImportError(msg) from exc
    except ImportError as exc:
        raw = str(exc)
        if "Library not loaded:" in raw and "qt@5" in raw:
            msg = (
                "EnvPool is installed but failed to load native libraries because Qt5 "
                "is missing on macOS.\n\n"
                "Install Qt5 and retry:\n"
                "  brew install qt@5\n\n"
                "If the linker still cannot find Qt, launch with:\n"
                "  DYLD_FRAMEWORK_PATH=/opt/homebrew/opt/qt@5/lib "
                "uv run benchmarking/gfootball_run_to_score_envpool/run.sh --device cpu --wb\n\n"
                "Original error:\n"
                f"{raw}"
            )
            raise ImportError(msg) from exc
        ...
        msg = (
            "EnvPool import failed due to a native extension/runtime dependency issue.\n"
            "This is usually a missing system library (not a Python package problem).\n\n"
            "Original error:\n"
            f"{raw}"
        )
        raise ImportError(msg) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train DQN on EnvPool Google Research Football run_to_score.",
    )
    parser.add_argument(
        "manifest",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: benchmarking/gfootball_run_to_score_envpool/checkpoints/"
            "gfootball_academy_run_to_score_elite_DQN.pt)."
        ),
    )
...
def main() -> None:
    """Run local DQN training on EnvPool GFootball run_to_score."""
    args = parse_args()
