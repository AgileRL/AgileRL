...
Quantization stores a model's weights at lower numerical precision than the
native ``bfloat16``/``float16`` — typically 8-bit integers or 4-bit floats —
instead of 16 bits per value. AgileRL uses
`bitsandbytes <https://github.com/bitsandbytes-foundation/bitsandbytes>`_ to
quantize the Hugging Face trainer, and pairs it with a bitsandbytes-quantized
colocated `vLLM <https://docs.vllm.ai/>`_ rollout engine for matching base
...
* **Negligible quality cost with QLoRA.** Because only the LoRA adapters are
  ...
  quantization of the base recovers full-fine-tuning quality in practice —
  this is the `QLoRA <https://arxiv.org/abs/2305.14314>`_ recipe.
...
* between rollout and **training**, vLLM is slept with its native
  ``sleep(level=1)`` — the base is backed up to host RAM and the KV cache (plus
  other recomputable allocations) is freed — and the trainer's base is moved
  back onto the GPU for the forward/backward. ``wake_up()`` restores vLLM's base
  before the next rollout.

This relies on vLLM ``>= 0.22``, whose ``sleep(level=1)`` round-trips the base
through host RAM losslessly for **both** dense (BF16/FP16) and bitsandbytes
4-bit weights. (Earlier vLLM could not restore a 4-bit base in place, which is
why a "keep the base resident" sleep patch used to exist; it — and the
zero-copy weight-sharing it supported — have been removed in favour of this
simpler path.) It works for quantized and dense bases alike and happens
automatically for colocated rollouts; there is nothing to configure.
...
preprocessing and attaches the trainable LoRA adapters on top — i.e. QLoRA.
...
training therefore composes with ZeRO (including ZeRO-3) and gradient
checkpointing — no extra configuration is required beyond the usual
...
``VLLMConfig.quantization`` is forwarded verbatim to ``vllm.LLM(quantization=...)``
— any other vLLM-supported backend (e.g. ``"awq"``, ``"gptq"``, ``"fp8"``,
``"compressed-tensors"``) is accepted but is not validated by AgileRL. Those
...
passthrough (forwarded verbatim to ``vllm.LLM(kv_cache_dtype=...)``) for users
on FP8-capable hardware (compute capability 8.9+ — Ada Lovelace, Hopper,
Blackwell). AgileRL does not validate any value here; vLLM emits its own
