...
.. code-block:: python
  ...
  memory = ReplayBuffer(action_dim=2, memory_size=10000, field_names=field_names)

  agent = DDPG(state_dim=8, action_dim=2, one_hot=False)   # Create DQN agent
...
.. code-block:: python
  ...
  agent = DDPG(states_dim=8, action_dim=4, one_hot=False, net_config=NET_CONFIG)   # Create DQN agent  
