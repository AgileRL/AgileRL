...
:class:`~agilerl.llm_envs.rollout_env.RolloutEnv` over a URL, so you can **host the environment on
...
.. code-block:: text
    ...
    ┌───────────────────────┐                 ┌──────────────────────────────┐
    │ BatchRolloutEnv        │  ws session 0   │ make_env() -> Sudoku board 0 │
...
.. code-block:: bash
    ...
    pip install "agilerl[llm,gem]"    # 'gem' pulls in gem-llm for the Sudoku env
...
In the run manifest, the environment is declared with ``env_url`` instead of an in-process ``env_name``.
