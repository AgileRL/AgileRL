...
from tests.test_memory.test_formulas import MOE_TINY, QWEN_05B
...
def test_colocated_ppo_qlora_does_not_require_a_named_quant_variant(device):
    # Default ModelSpec only declares "base"; training QLoRA is quantization +
    # kbit_prepared, not model.variant("nf4").
    model = ModelSpec(model_id="Qwen/Qwen2.5-0.5B-Instruct", arch=QWEN_05B)
    ...
    bf16 = estimate_generation(
        model,
        device,
        GenerationSettings(),
        train_settings=train.model_copy(update={"quantization": "none"}),
        colocated=True,
    )

    assert component(nf4, "trainer_residual").bytes_ > 0
    assert (
        component(nf4, "trainer_residual").bytes_
        < component(bf16, "trainer_residual").bytes_
    )
...
def test_training_settings_reject_chunk_rows_outside_the_fused_range():
    with pytest.raises(ValidationError, match="greater than or equal"):
        TrainingSettings(chunk_rows=64)
    with pytest.raises(ValidationError, match="less than or equal"):
        TrainingSettings(chunk_rows=4097)
    assert TrainingSettings(chunk_rows=128).chunk_rows == 128


def test_batch_size_advice_halves_update_rows_when_micro_batch_is_set(model):
    ...
    suggestions = advise(config, phase="training", top_n=None)
    batch_advice = [s for s in suggestions if "no-grad logprob batch" in s.action]

    assert batch_advice
    assert batch_advice[0].saves_bytes > 0
...
class TestDistributedTerms:
    ...
    def test_zero3_packed_backward_uses_microbatch_not_update_rows(self, device):
        ...
        shared = {
            ...
            "packed_moe_dispatch": "materialized",
            "micro_batch_size_per_gpu": 1,
            "max_model_len": 1024,
        }
        few = estimate_training(
            model,
            device,
            TrainingSettings(trajectories_per_update=8, **shared),
        )
        many = estimate_training(
            model,
            device,
            TrainingSettings(trajectories_per_update=64, **shared),
        )
        few_packed = component(few, "zero3_state").detail["packed_moe_backward"]
        many_packed = component(many, "zero3_state").detail["packed_moe_backward"]

        assert few_packed == many_packed > 0

        wider = estimate_training(
            model,
            device,
            TrainingSettings(
        ...
        assert component(wider, "zero3_state").detail["packed_moe_backward"] == (
            4 * few_packed
        )

    def test_contracted_packed_moe_charges_split_lora_not_w_eff(self, device):
        model = ModelSpec(model_id="moe-test", arch=MOE_TINY)
        settings = TrainingSettings(
            distributed="deepspeed",
            n_training_gpus=2,
        ...
        assert component(bar, "zero3_state").detail["packed_moe_backward"] == 0
    ...
    def test_zero3_packed_moe_keeps_checkpoint_saves_under_offload(self, device):
        model = ModelSpec(model_id="moe-test", arch=MOE_TINY)
        shared = {
            "distributed": "deepspeed",
            "n_training_gpus": 2,
        ...
        on_device = estimate_training(
            model, device, TrainingSettings(activation_offload=False, **shared)
        )
        offloaded = estimate_training(
            model, device, TrainingSettings(activation_offload=True, **shared)
        )

        assert (
            component(offloaded, "activations").detail["checkpoint_boundaries"]
            == component(on_device, "activations").detail["checkpoint_boundaries"]
        )
        assert any("activation_offload" in w for w in offloaded.warnings)
