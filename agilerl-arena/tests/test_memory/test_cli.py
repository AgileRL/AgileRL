...
def test_checkpoint_param_count_drops_a_tied_checkpoints_duplicate_lm_head(monkeypatch):
    # Tied checkpoints store lm_head and embed_tokens; from_pretrained re-ties them, so counting both invents 311 MiB.
...
def test_allow_oversize_downgrades_the_gate(oversize_path, capsys):
    """Override returns 0 and still prints the shortfall."""
    code = main(
...
def test_bar_never_exceeds_its_width(oversize_path, capsys):
    """Over-budget bar truncates at capacity; renormalising would hide a 3x overshoot."""
...
def test_missing_device_is_a_usage_error(manifest_path):
    assert main(manifest_path, config_path=TINY_CONFIG) == EXIT_USAGE


def test_unknown_gpu_is_a_usage_error(manifest_path, capsys):
    assert main(manifest_path, config_path=TINY_CONFIG, gpu="not-a-gpu") == EXIT_USAGE
    assert "unknown GPU" in capsys.readouterr().err


def test_unknown_gen_gpu_is_a_usage_error(manifest_path, capsys):
    assert (
        main(
            manifest_path,
