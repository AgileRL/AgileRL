...
def test_resolve_chunk_rows_mirrors_framework_heuristic():
    ...
    assert formulas.resolve_chunk_rows(151936, explicit=256) == 256
...
def test_moe_resident_gather_is_training_only_and_scales_with_routing():
    # MoE gathered expert copies outlive the block that made them; dense checkpointing assumes they do not.
...
def test_bnb_workspace_scales_with_gradient_tokens_and_geometry():
    # Per-token bnb workspace: 68/104/160/194 KiB on Qwen2.5-0.5B/1.5B/3B and Gemma 4 E4B.
    ...
    # Deeper costs more. Qwen2.5-0.5B is near the measured 68 KiB/token.
...
def test_kbit_prep_upcasts_a_tied_embedding_to_fp32():
    # On a tied model, k-bit prep upcasts the embedding table (lm_head is that table; counts.lm_head is 0).
...
def test_engine_overhead_tracks_depth_and_slots():
    # 512 MiB + 8 MiB per layer + 8 MiB per sequence slot. Held-out error 71 MiB over 182 points.
    ...
    # Qwen2.5-0.5B medians: 741 MiB on A100, 687 on L4 (sweeps around 8 slots). Device-independent.
...
def test_param_counts_reconciles_to_the_checkpoint_total():
    ...
    # Unattributed params are not quantizable.
...
def test_allocator_reserve_is_a_markup_on_allocated_bytes():
    # 7% is the median reserved/allocated ratio on training points.
...
#: Falcon-H1 runs attention and SSM in parallel in every block with no layout, so both counts are n_layers.
FALCON_H1 = {
    "num_hidden_layers": 24,
    "hidden_size": 2048,
...
def test_aligned_block_size_matches_vllm():
    """vLLM 0.26 A100 numbers: Nemotron-H SSM state is fp32, Falcon-H1 uses model dtype."""
...
def test_quantized_weight_bytes_include_hybrid_mamba(config):
    arch = ModelArch.from_hf_config(config)
    counts = formulas.param_counts(arch)
    assert counts.mamba > 0

    nf4 = formulas.weight_bytes(
        counts, "bf16", WeightVariant(name="nf4", quantization="nf4")
    )
    without_mamba = formulas.weight_bytes(
        replace(counts, mamba=0),
        "bf16",
        WeightVariant(name="nf4", quantization="nf4"),
    )

    # Arrange / Act above; Mamba stays at the checkpoint dtype, not nf4.
    assert nf4 - without_mamba == counts.mamba * 2


def test_nemotron_h_nf4_keeps_mamba_at_checkpoint_dtype():
    counts = formulas.param_counts(ModelArch.from_hf_config(NEMOTRON_H))
    mamba_bf16 = counts.mamba * 2

    # ~4.0B Mamba params → ~7.4 GiB bf16; the quantized path must still
    # charge that group.
    assert counts.mamba == pytest.approx(4.0e9, rel=0.05)
    assert mamba_bf16 == pytest.approx(7.4 * GiB, rel=0.05)

    nf4 = formulas.weight_bytes(
        counts, "bf16", WeightVariant(name="nf4", quantization="nf4")
    )
    assert nf4 > mamba_bf16
...
def test_lora_param_count_follows_hybrid_layer_mix():
    ...
    # Assert — attention adapters on 4 layers, not 56.
...
def test_kv_cache_uses_global_head_dim_on_full_attention_layers():
    ...
    # Arrange / Act
    per_token = formulas.kv_cache_bytes_per_token(arch, "auto", "bf16")
    demand = formulas.kv_cache_demand_bytes(arch, "auto", "bf16", 8, 8192)

    # Assert — full-attention KV is 512-wide, not the sliding 256.
    ...
    sliding_only = formulas.kv_cache_bytes_per_token(
        arch.model_copy(update={"global_head_dim": None}), "auto", "bf16"
    )
    ...
    assert formulas.kv_cache_bytes_per_token(full, "auto", "bf16") == (
        2 * storing * 1 * 512 * 2
    )
    assert formulas.kv_cache_demand_bytes(full, "auto", "bf16", 8, 8192) == int(
        8 * 8192 * 2 * storing * 1 * 2 * 512
    )


def test_lora_input_casts_hold_only_the_widest_single_cast():
    # Checkpointed LoRA: one fp32 cast of the widest input (down_proj). SmolLM2-1.7B 8x4096/8192 = 1024 MiB.
    smol = ModelArch(
        n_layers=24,
        hidden_size=2048,
    ...
    # Attention-only never wraps down_proj; residual stream and attention output can bind.
...
class TestZero3PackedMoeBackwardBytes:
    def test_scales_with_microbatch_rows_and_seq_len(self):
        one = formulas.zero3_packed_moe_backward_bytes(MOE_TINY, 1, 1024, 2)
        four = formulas.zero3_packed_moe_backward_bytes(MOE_TINY, 4, 1024, 2)

        assert one == 1024 * formulas.ZERO3_PACKED_MOE_BACKWARD_BYTES_PER_ROW_TOKEN
        assert four == 4 * one

    def test_zero_when_not_materialized_moe(self):
        assert formulas.zero3_packed_moe_backward_bytes(QWEN_05B, 1, 1024, 2) == 0
        assert formulas.zero3_packed_moe_backward_bytes(MOE_TINY, 1, 1024, 0) == 0
        assert (
            formulas.zero3_packed_moe_backward_bytes(
                MOE_TINY, 1, 1024, 2, dispatch="contracted"
            )
            == 0
        )
