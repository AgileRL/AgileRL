...
def test_solve_micro_batch_size_is_the_last_value_that_fits(model, device):
    config = RunConfig(
        model=model,
        train_device=device,
        training=TrainingSettings(max_model_len=2048, micro_batch_size_per_gpu=1),
        generation=GenerationSettings(max_model_len=2048),
    )
...
def test_unknown_field_is_a_value_error(model, device):
...
def test_solve_max_model_len_keeps_training_and_generation_in_lockstep(model, device):
    config = RunConfig(
        model=model,
        train_device=device,
        training=TrainingSettings(max_model_len=512, micro_batch_size_per_gpu=1),
