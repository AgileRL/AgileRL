...
class TestResolution:
    ...
    def test_compact_recurrent_ppo_name_implies_recurrent(self) -> None:
        validated = TrainingManifest.model_validate(
            {
                "algorithm": {"name": "RecurrentPPO"},
    ...
    def test_rainbow_n_step_comes_from_the_buffer(self) -> None:
        rainbow = copy.deepcopy(DQN)
        rainbow["algorithm"] = {"name": "RainbowDQN"}
        rainbow["replay_buffer"] = {
            "n_step_buffer": True,
            "n_step_buffer_args": {"n_step": 5},
        }
        validated = TrainingManifest.model_validate(rainbow)
        assert validated.algorithm.n_step == 5
    ...
    def test_colocated_user_vllm_config_sleeps(self) -> None:
        validated = TrainingManifest.model_validate(
            manifest(
                GRPO,
                algorithm={"vllm_config": {"gpu_memory_utilization": 0.4}},
            )
        )
        assert validated.algorithm.vllm_config.sleep_mode is True
        assert validated.algorithm.vllm_config.gpu_memory_utilization == 0.4

    def test_vllm_rejects_embedding_lora_targets(self) -> None:
        with pytest.raises(ValidationError, match="embed_tokens"):
            TrainingManifest.model_validate(
                manifest(
                    GRPO,
                    network={
                        "lora_config": {"target_modules": ["q_proj", "embed_tokens"]}
                    },
                )
            )

    def test_async_rollout_rejects_embedding_lora_targets(self) -> None:
        with pytest.raises(ValidationError, match="embeddings"):
            TrainingManifest.model_validate(
                manifest(
                    ...
                    network={
                        "lora_config": {"target_modules": ["q_proj", "embeddings"]}
                    },
                )
            )
