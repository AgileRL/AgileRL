...
VLLM_UNSUPPORTED_LORA_SUFFIXES: frozenset[str] = frozenset(
    {
        "conv1d",
        "embed_tokens",
        "embeddings",
...
"""Module suffixes PEFT wraps but vLLM's LoRA adapter loader cannot serve."""


def _lora_target_module_entries(target_modules: Any) -> tuple[str, ...]:
    """Normalize a PEFT ``target_modules`` value into candidate module names."""
    if isinstance(target_modules, str):
        return (target_modules,)
    if isinstance(target_modules, (list, tuple, set, frozenset)):
        return tuple(entry for entry in target_modules if isinstance(entry, str))
    return ()


def validate_vllm_lora_target_modules(
    target_modules: Any,
    *,
    served_by_vllm: bool,
    unsupported_suffixes: Iterable[str] = VLLM_UNSUPPORTED_LORA_SUFFIXES,
) -> None:
    """Reject LoRA targets vLLM's adapter loader cannot serve.

    :param target_modules: The manifest's ``lora_config.target_modules``.
    :param served_by_vllm: Whether vLLM serves the policy for this run.
    :param unsupported_suffixes: Module suffixes vLLM refuses to load.
    :raises ValueError: If a target module ends in an unsupported suffix.
    """
    if not served_by_vllm:
        return
    denied = frozenset(unsupported_suffixes)
    matched = {
        entry: suffix
        for entry in _lora_target_module_entries(target_modules)
        if (suffix := entry.rsplit(".", 1)[-1]) in denied
    }
    if not matched:
        return
    offenders = sorted(
        entry if entry == suffix else f"{entry} (suffix {suffix})"
        for entry, suffix in matched.items()
    )
    msg = (
        f"network.lora_config.target_modules {offenders} name embedding or "
        "convolution modules that vLLM's LoRA adapter loader cannot serve; its "
        "supported set covers the linear family only, so the adapter would fail "
        "to load at the first weight sync instead of here. Remove them, or drop "
        "the suffix from VLLM_UNSUPPORTED_LORA_SUFFIXES once vLLM serves it. "
        "Training without vLLM is unaffected."
    )
    raise ValueError(msg)
