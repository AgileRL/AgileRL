...
class AlgorithmRegistry:
    ...
    def resolve(self, name: str) -> str | None:
        """Return the registered spelling of *name*, treating spaces as optional.

        :param name: Algorithm name from a manifest (``RecurrentPPO`` or
            ``Recurrent PPO``).
        :type name: str
        :returns: The registry key, or ``None`` if nothing matches.
        :rtype: str | None
        """
        if name in self._entries:
            return name
    ...
    def create(self, name: str, **fields: Any) -> AlgoSpec:
        ...
        if resolved is None:
            supported = ", ".join(sorted(self._entries))
            msg = (
                f"No registry entry for algorithm {name!r}. Registered: {supported}"
            )
            raise KeyError(msg)
        spec_cls = self._entries[resolved]
        implied = spec_cls.alias_implies.get(resolved, {})
        for field, value in implied.items():
            if field in fields and fields[field] != value:
                msg = (
                    f"algorithm.name {resolved!r} implies {field}={value!r}, "
