...
```
peak(model_spec, device_spec, settings) -> {component: bytes}   # per phase
```
...
exactly the settings the run would train with, never a hand-assembled copy.

The estimator is validated against measured peaks. The public package does
not ship that measurement corpus.
...
caller has it (`arena memory estimate` reads it from Hugging Face Hub
safetensors metadata — still metadata, still no weight download), and
`param_counts` books the difference to `ParamCounts.unattributed`. Weight bytes then come out exact
...
| module | role |
|---|---|
| `specs.py` | parse `config.json` -> geometry; `TrainingSettings` / `GenerationSettings` and device schemas |
...
| `advice.py` | rank the cheapest setting changes when a bar is over budget |
| `solver.py` | invert one field: largest value that still fits |
| `cli.py` | pre-submission gate (`arena memory estimate`) and field solve (`arena memory solve`) |

`arena memory solve FIELD` holds every other input fixed and binary-searches
...
Invertible fields: `max_model_len`, `max_num_seqs`, `micro_batch_size_per_gpu`.

### Why there is no calibration layer

There is no fitted correction. A per-(model, device) fit on 406 measurements
cuts in-sample training error from 3.69% to 2.66% and raises
leave-one-model-out error from 4.8% to 6.8–9.2%. For a gate that must size
arbitrary checkpoints, that is a liability. Accuracy comes from finding
missing physical terms, not from coefficients.
...
ground truth yet, and the estimate says so in its warnings.

## Distributed terms — analytic, unmeasured
...
- **Distributed-job overhead** is a measured ~50 MiB per GPU, charged on
  both phases for an Arena submission. A separately observed ~3.2 GiB gap
  on one distributed job remains unexplained and is not charged.
...
Paired runs on both Gemma sizes, each with its own `eager` control:
...
arithmetic for it is right — while SDPA and FlexAttention are within 156 MiB
...
- **The `eager` term is too small by about half.** Against these six points the
  ...
  so whatever `eager` costs beyond three copies is unmodelled.

The estimator does not charge an `S²` matrix on SDPA. A term that scaled as
`rows x S²` on windowed SDPA flipped sign across micro-batch (Gemma 4 E2B at
seq 4096: −9.4% at micro-batch 1 and +10.2% at micro-batch 8 with it;
−15.6%/−12.1% without). A consistent bias can be hunted; a sign flip cannot.
...
The recurrent state's dtype is resolved **per model** by vLLM, not from the
config: Nemotron-H returns `(bfloat16, float32)` and Falcon-H1
`(bfloat16, bfloat16)`. That doubles Nemotron's page and its block size with
it — predicting 672 where vLLM logged 1,312 — so `mamba_ssm_state_dtype`
defaults to fp32, the larger of the two, because a memory estimate must not
read low. The rounding granularity is 32, the attention backend's kernel
block alignment, not vLLM's default 16.
...
- **The 8k agreement is partly luck.** At that context the profiling peak
  ...
  total comes out close because the clamp removes a term that was too large. The
...
A sleeping-engine floor is sampled per point — NVML at the moment the
training window opens — and training points are re-based onto the sweep
median so the fitted quantity is what the trainer adds, not whatever the
device happened to hold. Generation points are not re-based; their target is
the raw peak.
...
a missing term. Generation is deliberately **not** re-based in response:
doing so helps OLMoE and costs every model whose floor is stable, taking the
pooled mean from 3.19% to 3.38%. The raw peak is the right target when the
...
bytes nor the device's, and two runs at an identical config still differ by
748 MiB. Whatever that is, it is pinned by vLLM's CuMem allocator and varies by
...
The constant is stack-dependent. On torch 2.11.0+cu130 the trainer-only
reserved-over-allocated ratio — reconstructed per point by netting the
engine's CuMem phantom out of the torch counters — runs 8–25%, while the
model's *allocation* arithmetic reconstructs to within a few percent at the
very corners that read low. **5%** is the largest value every stored
fixture's drift bands tolerate (0.06 tips SmolLM2@L4 over the 8% mean band);
the pooled corpus pays 4.41% → 4.85% training mean for it, all of it on the
...
Direct attribution of a Gemma 4 E2B snapshot found **49 MiB** of simultaneous
mask residency — three bool `(1,1,4096,4096)` masks, allocated and freed per
layer — against the 2,240 MiB an `S²` windowed-mask term would claim. The
estimator does not charge that term.
...
| Caching-allocator slack | 5% of the torch-side subtotal | the device is charged reserved, not allocated, bytes; measured 8–25% on torch 2.11.0+cu130, and 5% is what the drift bands tolerate. Training only — vLLM's CuMem pool is reserved up front. See "Allocator reserve" |

What is *not* a training memory setting, and worth teaching in the UI:
...
Tabulating memory is infeasible (six settings × four levels is 4096 runs per
model, and sequence length is continuous) and unnecessary: the analytic form
carries the shape. There is no per-model fitted constant.
...
point in it is consistent with the current tree.

Replay of that corpus through the closed-form core on CPU is the accuracy
claim. Constants drift with framework changes — anything touching the fused
kernels, checkpointing or the vLLM wiring moves them — and that replay is
the check, since the estimator keeps returning plausible numbers either way.

## Porting notes

- Component `key` strings are the contract with the widget and the
  Arena API; change them only with a `schema_version` bump.
- `estimate_run(...).model_dump(by_alias=True)` is exactly the widget
  payload; `advise()` is the "you are N GiB over, cheapest fixes" list,
  computed by re-running the estimator under candidate setting mutations so
  it can never disagree with the bars.

## Measuring memory under vLLM
...
`learn()` sleeps the engine itself, so a measurement window opened around
`learn` samples the still-awake engine and reports the *rollout* footprint
as the training peak. Sleep explicitly first (idempotent), then open the
window. Getting this wrong overstated training by **5×** (12.08 vs 2.34 GiB
on a 0.5B).

Two wrong measurements can agree. The two bugs above both inflate the
training figure and sit within a few percent of each other. Cross-check any
device-level number against a physical sanity bound — a 0.5B model cannot
need 12 GiB to train a LoRA adapter.

NVML polling can still miss a spike. The torch-growth term covers the brief
backward peak between samples.

vLLM's CuMem allocator is process-global and permits one engine per process.

## Current modelling facts
...
- **A per-block transient peaks at the widest block, not the mean one.**
  ...
  Uniform stacks are unchanged; E2B's fixture moved −11.1% → −5.7% signed
  on this.
...
claim. Mean absolute error per fixture:
...
the geometry paths the Qwen models never touch. Its training is the best
fixture in the corpus and its generation the worst: the engine on the
measured stack decouples from the `gpu_memory_utilization` budget (see the
open list), which no budget arithmetic reproduces.

The measured axes span sequence length, micro-batch, group size, LoRA rank
*and* `gpu_memory_utilization` (0.21→0.56 on the A100 SmolLM2 sweep,
0.30/0.60 holdouts on the L4 fixtures), so the corpus is not pinned to the
engine budget it was measured under.
...
allocator snapshot attributes the *peak instant* per call site. On
Qwen2.5-0.5B at seq=4096, micro-batch 8, group 16:
...
Recording must be scoped to the training window (otherwise the peak belongs
to vLLM's start-up KV allocation), and it must be read from the event trace
rather than the final block list (by the time `learn` returns, every
activation has been freed).
...
(or is consistent with) the current working tree.** The estimator models the
current framework training path — no per-fixture gating, no version
switches; a fixture's `framework_versions` is provenance only.
...
SFT and DPO sweeps, and three A100 re-sweeps (21 training + 21 generation
points each, scored against predictions computed the way the CLI computes —
`config.json` parsed fresh, weights reconciled against the checkpoint's
parameter count, no realised sizes):
...
After the allocator-reserve (5%) and widest-block rules, the re-swept
fixtures replay at 4.5% / 6.2% / 1.5% training mean. Gemma 4 E2B is the
best-predicted training fixture in the corpus, and the pooled worst point
is an 18.0% over-prediction, which is the direction the gate can afford.

Generation: MoE dispatch copies are excluded engine-side (vLLM's fused-MoE
routes in-kernel; curated OLMoE generation 1.56% mean), and the CUDA-graph
pool constant follows vLLM's own logged 40–93 MiB rather than a 2 GiB guess
— at 2 GiB it drove the computed pool negative on seven measured-fine
configurations and raised false OOM warnings against them.
...
fine. That gap was the expert gather outliving its checkpoint block; with
that term the held-out MoEs are 7.26% / 12.86% for nothing on the corpus.

Sliced by `seq_len`, `micro_batch`, `group_size` or `lora_rank` the signed
error is flat to within a point; sliced by model it still spreads —
SmolLM2's backward instant over-charges (255 KiB per grad token measured
against 316 charged) where Qwen's matches (156/160) — and that spread is
per-model allocation behaviour the closed form does not carry. Fitting it
would put a per-model coefficient back in. The remaining attributions need
allocator timelines at named corners, not more sweeping.

Open, in rough priority order:

- **Quantized training is validated end to end, on one model.** k-bit
  preparation upcasts a *tied* `lm_head` (the embedding table). The
  per-token bitsandbytes workspace is `40 KiB/token + 1.5 bytes per
  (n_layers x hidden)` (R² 0.982, held to 12.9 KiB/token
  leave-one-model-out). `weight_bytes` adds those upcasts on top of a
  realised quantized size. The nf4 sweep scores **4.3% mean / 10.9% worst /
  +0.2% signed** against 3.4% for the same model unquantized. Still one
  model on one device; the 1.5B/3B/E4B paired points check the *delta*,
  not the absolute bar.
...
- **Gemma 4's training gap is closed; its engine is the open item.**
  The SDPA timelines count **one** live decoder block at the training peak,
  exactly as modelled. Remaining training residual decomposes into the
  stack-level allocator reserve (5%, see "Allocator reserve") and the
  widest-block rule (`peak_mlp_width_factor` / `peak_qkv_dim`: E2B's
  double-wide-MLP global-attention blocks are what a per-block transient
  binds on). E2B training replays at 1.5% mean / 3.7% worst. **Generation**
  is the residual problem: measured engine residency does not track
  ...
  where the same engine under vLLM 0.26 filled whatever it was given, and
  engine *construction* transiently peaks at 1.76× the serving bar (24 GiB
  at 16 slots × 4096). Three independent instruments agree (generation
  NVML, the training-window CuMem phantom, the startup phase). The bar
  warns on every multimodal checkpoint; deciding *which* engine allocation
  does this (encoder cache and multimodal profiling are the candidates)
  needs an engine-side trace. OLMoE's remaining gap is training-side only:
  mb8/g16 over-prediction from the known `MOE_GATHER_RESIDENT_BLOCKS = 4`
  trade — corpus-neutral and held-out-optimal against granite, so the
  constant stands.
- **DP / ZeRO-3 / distributed-job terms are analytic and unmeasured** (see
  "Distributed terms"): the shapes are stated so a measurement can falsify
  them, and the 3.2 GiB distributed-job observation is still unexplained.
- **sft and dpo are measured and in the corpus; ppo is half-measured.**
  The estimate includes three mechanisms verified against this tree's
  source: the engine-less trainer's own library workspaces (583 MiB,
  ...
  *recompute-only* wherever the forward runs under `_amp_ctx` — the cast
  ...
  through the rollout; its *training* points are parked — those runs sent
  one prompt and PPO samples one completion per prompt, so every update had
  one trajectory and the batch axes were never exercised.
- **First-step peaks are not steady-state peaks.** Every fixture is measured
  ...
  and the Qwen points show the same signature at token-heavy corners
  ...
  re-measuring one sweep at warmup 1, not by softening the terms.
- **vLLM's fused-MoE workspace is unmodelled.** Remaining OLMoE generation
  error is −0.5% to −5.3%, worst where `max_num_batched_tokens` is largest —
  the shape of the kernel's chunked intermediate caches
  (`VLLM_FUSED_MOE_CHUNK_SIZE`). The estimate warns on MoE generation.
  It does not guess the chunk size.
...
- **Some fixtures still hold only 3–5 training points** (Qwen2.5-1.5B/3B/7B,
  the L4 Qwen2.5-0.5B, Gemma 4 E4B) — the backward-bound corners only.
  Qwen2.5-0.5B, SmolLM2 and E2B on A100 are the dense fixtures. For the
  rest the per-fixture mean drift band applies only to phases holding 8+
  points, with the worst-point and pooled-mean bands guarding everything.
  E4B's generation points are additionally 0.26-engine vintage where
  current runs use 0.24 — the two stacks diverge on its sibling — so treat
  E4B generation accuracy claims as indicative.
- **The dense long-context generation under-read did not reproduce on the
  later stack.** The −13.4% cluster (SmolLM2-1.7B on L4 at 16 slots × 4096)
  survives only in the L4 fixture; the A100 points at the same settings
  and gmu up to 0.56 read −0.4..−0.8%. The L4 points stay as measured;
  whether the gap was the engine vintage or the device class is
  undecidable without an L4 re-measure.
