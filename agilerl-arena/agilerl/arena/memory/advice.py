...
def _training_candidates(
    ...
    if t.batch_size > 1:
        halved_batch = max(1, t.batch_size // 2)
        # Peak is grad_rows (capped by trajectories), not batch_size. Keep
        # the update row count in step with the halved prompt batch.
        halved_update = max(1, t.trajectories // 2) * t.dp_world_size
        candidates.append(
            (
                f"Halve the no-grad logprob batch ({t.batch_size} -> {halved_batch})",
                with_training(
                    batch_size=halved_batch,
                    trajectories_per_update=halved_update,
                ),
            )
        )
