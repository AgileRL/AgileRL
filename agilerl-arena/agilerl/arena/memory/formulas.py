...
#: Overhead for an awake engine. Distinct from the sleeping-floor formula:
#: measured with the engine asleep, and substituting that formula
...
def param_counts(arch: ModelArch, n_params: int | None = None) -> ParamCounts:
    ...
    given, the difference against the analytic sum goes into
...
def lora_param_count(
    ...
    ``nn.Linear`` — attention q/k/v/o, dense MLP matrices, and Mamba in/out
    projections — but never the embedding or ``lm_head``. Each wrapped
    ``(out, in)`` linear adds ``rank * (in + out)``. Charged on the same
    layer subsets as :func:`param_counts`: attention adapters on
    :attr:`ModelArch.attention_layers`, MLP on :attr:`ModelArch.mlp_layers`,
    Mamba on :attr:`ModelArch.n_mamba_layers`. Fused MoE experts are not
    ``nn.Linear``, so a routed stack adds no MLP term unless it also has
    dense MLP blocks.
    """
    h, dh = arch.hidden_size, arch.head_dim
    q_dim = arch.n_heads * dh
...
def weight_bytes(
    ...
    Mamba mixer parameters stay at the checkpoint dtype: they are held out of
    :attr:`ParamCounts.quantizable`, and k-bit preparation does not upcast them.
    """
    ...
    return int(
        ...
        + counts.mamba * base_bytes
        + counts.unattributed * base_bytes
    )


def kv_storing_layers(arch: ModelArch) -> int:
    """Attention layers that keep their own K/V, not a shared earlier cache."""
...
def kv_layer_head_dims(arch: ModelArch) -> tuple[float, int, int]:
    ...
    Gemma 4 stores a wider head on full-attention layers (512 vs 256). With
    no sliding window every storing layer uses the full-attention width.
    """
...
def kv_cache_demand_bytes(
    ...
    the window size, and are charged at the sliding head width; full-attention
    layers keep the full sequence at :attr:`ModelArch.global_head_dim`.
    """
    kv_bytes = (
        DTYPE_BYTES[weight_dtype] if kv_dtype == "auto" else DTYPE_BYTES[kv_dtype]
    )
...
def resolve_chunk_rows(vocab_size: int, explicit: int | None = None) -> int:
    ...
    A fixed memory budget, so a function of vocabulary alone. An explicit
    value passes through; range-check it at construction so a bad setting is
...
def lora_input_cast_bytes(
    ...
    Under gradient checkpointing the casts do not accumulate: the block is
    recomputed as one unit, so the instant holds the widest single cast
    (MLP down-projection on the intermediate):
    ...
    Observed is the timeline's ``adapters`` excursion (PEFT allocates inside
    the LoRA layer).
    """
    h = arch.hidden_size
...
#: Extra device memory charged when ``orchestrated`` is set: ~50 MiB of
#: per-process job overhead on an L4 versus a bare process.
...
#: ZeRO-3 backward surcharge on a packed-MoE hybrid, per gradient-microbatch
#: token. Fitted on Nemotron 3.5 Lightning 30B-A3B (A100-80). Carrier is the
#: gathered expert leaf's backward (re-gathered weights, PEFT effective
#: copies, dispatch intermediates) for the rows in one ``_loss`` /
#: ``_backward_pass``. One model, one device: provisional.
...
def zero3_packed_moe_backward_bytes(
    ...
    ``grad_rows`` is the gradient micro-batch GRPO/CISPO slices into ``_loss``
    before ``_backward_pass``. Accelerate/DeepSpeed ``backward`` frees that
    graph before the next micro-batch; only those rows are live. Zero under
    contracted dispatch (the low-rank delta is formed by contraction, never
    the effective weight), when nothing targets the packed experts, or off
    ZeRO-3 — ZeRO-2 measured no surcharge on the same model.
    """
    if dispatch != "materialized" or not packed_matrices or not arch.is_moe:
        return 0
    return int(grad_rows * seq_len * ZERO3_PACKED_MOE_BACKWARD_BYTES_PER_ROW_TOKEN)
...
def split_moe_lora_recompute_bytes(
    ...
    """Split packed-expert LoRA residual workspace during checkpoint recompute.

    ``RoutedExpertsLoraWrapper`` runs the native experts forward, then
    ``_routed_lora_residual``: a top-k gather, per-expert ``F.linear`` plus
    ``torch.cat`` (chunks still live), and fp32 LoRA GEMMs. Native
    :func:`block_recompute_bytes` does not include that second pass. Zero
    when dispatch is materialized (PEFT builds ``W_eff`` instead) or nothing
    targets packed experts.
    """
    if dispatch != "contracted" or not packed_matrices or not arch.is_moe:
        return 0
    top_k = arch.n_experts_per_tok or 1
    routed = grad_rows * seq_len * top_k
...
def zero3_persistent_state_bytes(
    ...
    if (
        ...
        and packed_matrices
        and n_experts is not None
        and n_experts > 1
    ):
        expert_inter = arch.expert_intermediate_size or arch.intermediate_size
        w_eff = int(packed_matrices * n_experts * expert_inter * arch.hidden_size)
...
def zero3_gather_bytes(counts: ParamCounts, arch: ModelArch, dtype: str) -> int:
    ...
    Stage 3 shards every parameter and all-gathers roughly two modules in
    flight, so twice the larger of the embedding table and the widest decoder
    layer is charged. On a block-exclusive MoE hybrid the sparse block is one
    leaf and can be more than double a mean layer.
    """
