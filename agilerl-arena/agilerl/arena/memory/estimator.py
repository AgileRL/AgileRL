...
def _engine_terms(
    model: ModelSpec, device: DeviceSpec, settings: GenerationSettings
    ...
    terms = {
        ...
        "mamba_state": formulas.mamba_state_bytes(
            ...
            settings.mamba_cache_mode,
            settings.max_model_len,
            formulas.aligned_kv_block_size(
                arch, settings.kv_cache_dtype, settings.weight_dtype
            ),
...
def estimate_training(
    ...
    counts = formulas.param_counts(arch, model.n_params)
    act_bytes = DTYPE_BYTES[settings.weight_dtype]
    s = settings.max_model_len
    warnings: list[str] = []

    attn_impl = formulas.resolve_attn_implementation(
        settings.attn_implementation, device.flash_attn_installed
    )
    ...
    if settings.beta == 0.0 and settings.algorithm not in ("sft", "dpo"):
        warnings.append(
            ...
            "no-grad pass still builds that row unconditionally, so the run "
            "pays for one extra row of activations."
        )
    if settings.algorithm == "ppo":
        warnings.append(
            ...
            "prompt, and the measurement sent one prompt — so multi-row updates "
            "carry no measured ground truth yet."
        )
    if settings.distributed == "deepspeed" and settings.dp_world_size > 1:
        if settings.zero_stage == 3:
            warnings.append(
                f"DeepSpeed ZeRO-3 over {settings.dp_world_size} GPUs: the "
            ...
            warnings.append(
                f"DeepSpeed ZeRO-{settings.zero_stage} over "
                f"{settings.dp_world_size} GPUs: the sharding terms are "
    ...
    if (
        ...
        warnings.append(
            ...
            "checkpointed backward, not PEFT effective weights."
        )
    if not flash_like:
        warnings.append(
    ...
    kbit = settings.quantization != "none"
    ...
    shards = settings.dp_world_size
    if settings.distributed == "deepspeed" and settings.zero_stage == 3 and shards > 1:
        base = base / shards + formulas.zero3_gather_bytes(
            counts, arch, settings.weight_dtype
        )
    ...
    packed_shards = (
        shards
        if settings.distributed == "deepspeed" and settings.zero_stage == 3
        else 1
    )
    adapters = (
        lora_params * settings.n_resident_adapters + value_head_params
    ) * adapter_bytes_per_param + (
        packed_lora_params
        * settings.n_resident_adapters
        * DTYPE_BYTES[settings.weight_dtype]
        / packed_shards
    )

    trained_params = (
        lora_params + packed_lora_params
    ) * settings.n_trained_adapters + value_head_params
    ...
    opt_state = (
        0.0
        if settings.distributed == "deepspeed"
        else trained_params
        * formulas.optimizer_bytes_per_trainable_param(settings.distributed)
        / shards
    )
    ...
    grad_rows = settings.grad_rows
    graph_rows = settings.grad_graph_rows
    zero3_active = (
        settings.distributed == "deepspeed"
        and settings.zero_stage == 3
        and settings.dp_world_size > 1
    )
    packed_moe = bool(settings.lora_packed_target_matrices and arch.is_moe)
    ...
    if settings.activation_offload:
        if zero3_active and packed_moe:
            # save_on_cpu does not move tensors saved under a ZeRO-3 gathered
            # MoE leaf; the checkpointed backward still peaks on device.
            warnings.append(
                "activation_offload does not shrink the ZeRO-3 packed-MoE "
                "backward: the gathered expert leaf's checkpointed "
                "activations stay on device."
            )
        else:
            saved = 0
    ...
    # live for backward. ``_amp_ctx`` disables them on the shared forward,
    # but checkpoint recompute restores the flag — so one block's casts
    # reappear for ``grad_rows``. SFT's loss path stays outside ``_amp_ctx``
    # and keeps its forward casts. See
    # :attr:`TrainingSettings.lora_casts_recompute_only`.
    cast_rows = grad_rows if settings.lora_casts_recompute_only else graph_rows
    ...
    nograd_rows = settings.grad_rows * settings.n_adapter_rows
    nograd_pass = (
    ...
    rollout = 6 * settings.trajectories * s * 4

    # Packed-expert W_eff / leaf gather live only for the current micro-batch;
    # GRPO's learn loop backprops one slice then frees the graph.
    zero3_backward = (
    ...
    engine_residual = _engine_reservation_bytes(
        colocated and settings.uses_generation_engine, arch, settings.group_size
    )
    ...
    trainer_libs = (
        0 if settings.uses_generation_engine else formulas.TRAINER_LIB_OVERHEAD_BYTES
    )
    ...
    components = (
        ...
        _component(
            ...
            note=(
                "Measured at device level on a packed-MoE hybrid: one "
                "micro-batch's checkpointed backward of gathered expert "
                "leaves plus partition caches, reduce buckets and standing "
        ...
        _component(
            ...
            device.context_bytes + rollout + job_overhead + trainer_libs,
            detail={
...
def estimate_generation(
    ...
    train_settings: TrainingSettings | None = None,
    colocated: bool = False,
    orchestrated: bool = False,
) -> PhaseBreakdown:
    ...
    terms, batched_tokens, sampler = _engine_terms(model, device, settings)
    ...
    extrapolated = _context_extrapolation_warning(settings.max_model_len)
    if extrapolated:
        warnings.append(extrapolated)
    ...
    if arch.multimodal_tower_params or arch.per_layer_input_dim:
        # Gemma 4 E2B: engine residency did not track gpu_memory_utilization
        # (flat at weights + ~3.9 GiB across 0.28-0.44). Construction peaked
        # at 1.76x the serving bar at 16 slots x 4096.
        warnings.append(
    ...
    if arch.is_hybrid_ssm:
        ...
        if settings.mamba_cache_mode == "all":
            ...
            blocks = formulas.mamba_state_blocks_per_seq(
                "all", settings.max_model_len, block_size
            )
            warnings.append(
    ...
    if settings.kv_cache_dtype == "fp8" and not device.supports_fp8:
        warnings.append(
    ...
    if colocated and train_settings is not None:
        lora_params = formulas.lora_param_count(
            model.arch, train_settings.lora_rank, train_settings.lora_target_scope
        )
        trainer_residual = (
            lora_params
            * train_settings.n_trained_adapters
            * formulas.optimizer_bytes_per_trainable_param(train_settings.distributed)
            / train_settings.dp_world_size
        )
        if not train_settings.use_memory_efficient_params:
            counts = formulas.param_counts(arch, model.n_params)
            # QLoRA is settings.quantization on the base checkpoint, not a variant name.
            trainer_variant = WeightVariant(quantization=train_settings.quantization)
            value_head = arch.hidden_size + 1 if train_settings.uses_critic else 0
            trainer_residual += (
                formulas.weight_bytes(
                    counts,
                    train_settings.weight_dtype,
                    trainer_variant,
                    kbit_prepared=train_settings.quantization != "none",
                    tied_embeddings=arch.tied_embeddings,
                )
                + (lora_params * train_settings.n_resident_adapters + value_head)
        ...
        _component(
            ...
            detail={
                ...
                "state_blocks_per_seq": formulas.mamba_state_blocks_per_seq(
                    settings.mamba_cache_mode, settings.max_model_len
                ),
            },
            note=(
                f"{arch.n_mamba_layers} recurrent layers against "
                f"{arch.attention_layers} attention. Constant in context "
                f"length; mamba_cache_mode={settings.mamba_cache_mode!r}."
                if arch.is_hybrid_ssm
        ...
        _component(
            ...
            note=(
                (
                    ...
                    and not train_settings.use_memory_efficient_params
...
def recommend_engine_budget(
    ...
    """
    terms, _, _ = _engine_terms(model, device, settings)
...
def estimate_run(config: RunConfig) -> RunEstimate:
    ...
    # the quantization setting, not through a prequantized weight variant.
