...
#: Bounds an explicit fused-logprob ``chunk_rows``, and the auto-tune clamp.
...
#: Fallback CUDA context when the device itself has not been measured.
#: Midway between the two known devices (A100 501 MiB, L4 226 MiB). Prefer
#: ``DeviceSpec.cuda_context_bytes``.
...
def _layer_mix(text_cfg: dict[str, Any], n_layers: int) -> tuple[int | None, int]:
    ...
    Reads ``layer_types`` / ``layers_block_type``, Nemotron-H's
    ``hybrid_override_pattern``, Qwen3-Next's ``full_attention_interval``, or
    Falcon-H1's parallel mixer (both counts = ``n_layers``).
    """
...
class ModelArch(BaseModel):
    ...
    # Features that break the "one width per layer" assumption.
    ...
    # Hybrid SSM: Mamba-2 mixers have constant per-sequence state; attention
    # KV grows with context. Nemotron-Nano-9B-v2 has 4 attention layers of 56.
    ...
    mamba_n_heads: int = 0
    mamba_d_head: int = 0
    #: Dtype of the recurrent (SSM) state. vLLM resolves this per model:
    #: Nemotron-H keeps fp32, Falcon-H1 keeps the model dtype. Default fp32 so
    #: the estimate does not read low.
    ...
    def peak_mlp_width_factor(self) -> float:
        ...
        Per-block transients peak at the widest block, not the mean. Parameter
        totals keep using :attr:`mlp_width_factor`.
        """
        if not self.double_wide_mlp or not self.n_kv_shared_layers:
            return 1.0
        return 2.0
...
class TrainingSettings(BaseModel):
    ...
    #: How the packed-expert adapter path executes. ``"contracted"`` is the
    #: trainer default: split LoRA on packed experts (top-k gather, per-expert
    #: ``F.linear`` + ``cat``, fp32 GEMMs) with no effective-weight copy.
    #: ``"materialized"`` is PEFT building ``W_eff`` per targeted matrix.
    ...
    #: workspace, clamped to [FUSED_CHUNK_ROWS_MIN, FUSED_CHUNK_ROWS_MAX].
    chunk_rows: int | None = Field(
        default=None, ge=FUSED_CHUNK_ROWS_MIN, le=FUSED_CHUNK_ROWS_MAX
    )
    ...
    def uses_reference(self) -> bool:
        ...
        The fused no-grad pass builds the reference row even at beta=0, so a
        run still pays for it. The estimate says so in its warnings rather
        than assuming the skip.
        """
    ...
    def lora_casts_recompute_only(self) -> bool:
        ...
        ``_amp_ctx`` disables ``cast_input_dtype`` on the shared forward helpers.
        Checkpoint recompute runs after that context exits, so one block's
        casts reappear sized by ``grad_rows``. SFT's ``_sft_loss`` calls
        ``actor.forward`` outside ``_amp_ctx``, so its forward casts stay.
        """
        return self.algorithm != "sft"
