...
from agilerl.arena.client_datasets import (
    CSV_CONTENT_TYPE as CSV_CONTENT_TYPE,
)
from agilerl.arena.client_datasets import (
    DATASET_CATEGORIES as DATASET_CATEGORIES,
)
from agilerl.arena.client_datasets import (
    PARQUET_CONTENT_TYPE as PARQUET_CONTENT_TYPE,
)
from agilerl.arena.client_datasets import (
    DatasetClientMixin,
)
...
class ArenaClient(
    ...
    def _unauthorized_error(
        ...
        msg = (
            f"Session expired and could not be refreshed. Server response: {raw[:200]}"
        )
        return ArenaAuthError(
            msg,
    ...
    def _cli_capabilities_from_envelope(
        self, envelope: object
    ) -> dict[str, Any] | None:
        """Return the capabilities document from a JSON envelope, or None."""
        if (
    ...
    def _get_cli_capabilities(
        ...
        data = self._cli_capabilities_from_envelope(envelope)
