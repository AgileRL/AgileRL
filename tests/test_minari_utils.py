...
def test_minari_to_agile_buffer(dataset_id, env_id):
    ...
    memory = ReplayBuffer(
        action_dim, 10000, field_names=field_names, device=device)
...
def test_load_minari_dataset_errors(dataset_id):
    ...
    with pytest.raises(
        FileNotFoundError, match=f"No local Dataset found for dataset id {dataset_id}. check https://minari.farama.org/ for more details on remote dataset. For loading a remote dataset assign remote=True"
    ):
        minari_utils.load_minari_dataset(dataset_id)
    
    # test load a dataset absent in remote
    with pytest.raises(
        KeyError, match="Enter a valid remote Minari Dataset ID. check https://minari.farama.org/ for more details."
    ):
