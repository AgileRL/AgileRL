...
def test_peft_attach_sizes_lora_from_global_shape_without_gathering():
    ...
    def _parameter_owner(tensor):
        if tensor is fake_up:
            return experts, "gate_up_proj"
        if tensor is fake_down:
            return experts, "down_proj"
        return None

    with (
        patch.object(dmod, "DTensor", FakeDTensor),
        patch.object(dmod, "parameter_owner", side_effect=_parameter_owner),
        full_shape_views([fake_up, fake_down]),
    ):
...
ADAPTER_MAJOR_ROUTINGS = (
    ["__base__"] * 2 + ["actor"] * 2,
    ["__base__"] * 3 + ["actor"] * 3,
    ["__base__", "actor"],
    ["__base__"] * 2 + ["actor"] * 2 + ["critic"] * 2,
    ["ref"] * 4 + ["actor"] * 4,
)


def _inverse_permutation(src_rows: list[int]) -> list[int]:
    inverse = [0] * len(src_rows)
    for new_row, old_row in enumerate(src_rows):
        inverse[old_row] = new_row
    return inverse


class TestInterleavedSequenceAdapterPlan:
    def test_pairs_each_sequence_with_ref_and_actor(self) -> None:
        routing = ["__base__"] * 2 + ["actor"] * 2

        plan = interleaved_sequence_adapter_plan(routing)

        assert plan is not None
        interleaved, chunks, src_rows = plan
        assert interleaved == ["__base__", "actor", "__base__", "actor"]
        assert chunks == [(0, 2), (2, 4)]
        assert src_rows == [0, 2, 1, 3]

    def test_single_sequence_keeps_adapter_order(self) -> None:
        routing = ["__base__", "actor"]

        interleaved, chunks, src_rows = interleaved_sequence_adapter_plan(routing)

        assert interleaved == routing
        assert chunks == [(0, 2)]
        assert src_rows == [0, 1]

    def test_pairs_three_adapters_per_sequence(self) -> None:
        routing = ["__base__"] * 2 + ["actor"] * 2 + ["critic"] * 2

        interleaved, chunks, src_rows = interleaved_sequence_adapter_plan(routing)

        assert interleaved == ["__base__", "actor", "critic"] * 2
        assert chunks == [(0, 3), (3, 6)]
        assert src_rows == [0, 2, 4, 1, 3, 5]

    @pytest.mark.parametrize("routing", ADAPTER_MAJOR_ROUTINGS)
    def test_gathered_names_match_interleaved_routing(self, routing: list[str]) -> None:
        interleaved, _, src_rows = interleaved_sequence_adapter_plan(routing)

        assert [routing[i] for i in src_rows] == interleaved

    @pytest.mark.parametrize("routing", ADAPTER_MAJOR_ROUTINGS)
    def test_inverse_gather_restores_adapter_major_rows(
        self, routing: list[str]
    ) -> None:
        _, _, src_rows = interleaved_sequence_adapter_plan(routing)
        adapter_major = list(range(len(routing)))

        mixed = [adapter_major[i] for i in src_rows]
        restored = [mixed[i] for i in _inverse_permutation(src_rows)]

        assert restored == adapter_major
        assert sorted(src_rows) == adapter_major

    @pytest.mark.parametrize("routing", ADAPTER_MAJOR_ROUTINGS)
    def test_each_chunk_is_one_sequence_times_all_adapters(
        self, routing: list[str]
    ) -> None:
        interleaved, chunks, src_rows = interleaved_sequence_adapter_plan(routing)
        adapters = list(dict.fromkeys(routing))
        n_adapters = len(adapters)
        n_sequences = len(routing) // n_adapters

        assert chunks[0][0] == 0
        assert chunks[-1][1] == len(routing)
        assert [end for _, end in chunks] == [start for start, _ in chunks[1:]] + [
            len(routing)
        ]
        for start, end in chunks:
            assert end - start == n_adapters
            assert interleaved[start:end] == adapters
            sequences = [src % n_sequences for src in src_rows[start:end]]
            assert sequences == [sequences[0]] * n_adapters

    def test_returns_none_for_a_single_adapter(self) -> None:
        assert interleaved_sequence_adapter_plan(["actor"] * 4) is None
        assert interleaved_sequence_adapter_plan(["actor"]) is None

    def test_returns_none_when_run_lengths_differ(self) -> None:
        assert interleaved_sequence_adapter_plan(["ref"] * 2 + ["actor"] * 3) is None

    def test_returns_none_for_empty_routing(self) -> None:
        assert interleaved_sequence_adapter_plan([]) is None
