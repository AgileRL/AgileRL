...
from agilerl.modules import EvolvableMLP, EvolvableCNN, EvolvableMultiInput
from agilerl.algorithms.core.registry import HyperparameterConfig, RLParameter, NetworkGroup
from agilerl.algorithms.core import RLAlgorithm, MultiAgentRLAlgorithm, OptimizerWrapper
...
class DummyRLAlgorithm(RLAlgorithm):
    def __init__(self, *args, lr=True, **kwargs):
        ...
        num_outputs = self.action_space.n if isinstance(self.action_space, spaces.Discrete) else self.action_space.shape[0]
        if is_image_space(self.observation_space):
            self.dummy_actor = EvolvableCNN(
                self.observation_space.shape, num_outputs, channel_size=[3], kernel_size=[3], stride_size=[1]
                )
        elif isinstance(self.observation_space, (spaces.Box, spaces.Discrete)):
            num_inputs = self.observation_space.shape[0] if isinstance(self.observation_space, spaces.Box) else self.observation_space.n
            self.dummy_actor = EvolvableMLP(
                num_inputs, num_outputs, hidden_size=[8]
                )
        elif isinstance(self.observation_space, (spaces.Dict, spaces.Tuple)):
            self.dummy_actor = EvolvableMultiInput(
                self.observation_space, num_outputs, hidden_size=[8], channel_size=[3], kernel_size=[3], stride_size=[1]
                )
...
class DummyMARLAlgorithm(MultiAgentRLAlgorithm):
    def __init__(self, *args, **kwargs):
        ...
        def create_actor(idx):
            ...
            num_outputs = action_space.n if isinstance(action_space, spaces.Discrete) else action_space.shape[0]
            if is_image_space(obs_space):
                return EvolvableCNN(
                    obs_space.shape, num_outputs, channel_size=[3], kernel_size=[3], stride_size=[1]
                    )
            elif isinstance(obs_space, (spaces.Box, spaces.Discrete)):
                num_inputs = obs_space.shape[0] if isinstance(obs_space, spaces.Box) else obs_space.n
                return EvolvableMLP(
                    num_inputs, num_outputs, hidden_size=[8]
                    )
            elif isinstance(obs_space, (spaces.Dict, spaces.Tuple)):
                return EvolvableMultiInput(
                    obs_space, num_outputs, hidden_size=[8], channel_size=[3], kernel_size=[3], stride_size=[1]
                    )

        self.dummy_actors = [create_actor(idx) for idx in range(self.n_agents)]
        self.lr = 0.1
        self.dummy_optimizer = OptimizerWrapper(optim.Adam, self.dummy_actors, self.lr, multiagent=True)

        self.register_network_group(
            NetworkGroup(
                eval=self.dummy_actors,
                policy=True,
                multiagent=True
            )
        )
...
def test_initialise_multi_agent(observation_space, action_space):
    agent = DummyMARLAlgorithm(observation_space, action_space, agent_ids=["agent1", "agent2"], index=0)
    assert agent is not None
...
def test_population_multi_agent():
    ...
    population = DummyMARLAlgorithm.population(10, observation_spaces, action_spaces, agent_ids=["agent1", "agent2"])
    assert len(population) == 10
    for i, agent in enumerate(population):
        for j in range(2):
...
def test_incorrect_hp_config():
    with pytest.raises(AttributeError):
        ...
        agent = DummyRLAlgorithm(generate_random_box_space((4,)), generate_discrete_space(4), index=0, hp_config=hp_config)