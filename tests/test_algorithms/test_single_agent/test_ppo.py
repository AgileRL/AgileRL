...
class TestPPOInit:
    ...
    def test_ppo_optimizer_parameters(self, vector_space, discrete_space):
    ...
    def test_ppo_with_rollout_buffer(self, observation_space, action_space, request):
        observation_space = request.getfixturevalue(observation_space)
        action_space = request.getfixturevalue(action_space)
    ...
    def test_ppo_init_negative_target_kl_assert(self, vector_space, discrete_space):
        with pytest.raises(
            AssertionError,
    ...
    def test_ppo_init_head_config_path(self, vector_space, discrete_space):
        net_config = {
    ...
    def test_continuous_action_space_no_mask_buffer(self, vector_space):
...
class TestPPOGetAction:
    ...
    # Test PPO with hidden states
    def test_ppo_with_hidden_states_multiple_obs(self, vector_space, discrete_space):
        observation_space = vector_space
        action_space = discrete_space
        num_envs = 2
        ...
        obs = np.zeros(
            (num_envs, *observation_space.shape), dtype=observation_space.dtype
        )
...
class TestPPOLearn:
    ...
    def test_ppo_with_hidden_states_multiple_envs_collect_rollouts(self):
        num_envs = 2
        env = gymnasium.vector.SyncVectorEnv(
            [lambda: gymnasium.make("CartPole-v1")] * num_envs,
        )
    ...
    def test_rollout_buffer_flat_external_empty_returns_zero(
        self, vector_space, discrete_space
    ):
    ...
    def test_rollout_buffer_bptt_epoch_avg_kl_warning_branch(
        ...
        values = iter(
            [0.1, 1.0, 0.1]
        )  # minibatch KL, epoch avg KL, latest minibatch KL
        monkeypatch.setattr(
            "agilerl.algorithms.ppo.np.mean", lambda *_args, **_kwargs: next(values)
        )

        with pytest.warns(
            UserWarning, match="Average KL divergence .* exceeded target"
        ):
    ...
    def test_action_masks_applied_during_learning(
        self, action_space, vector_space, request
    ):
    ...
    @pytest.mark.parametrize("recurrent", [True, False])
    def test_action_masks_in_rollout_buffer_learn(
        ...
        for i in range(learn_step):
            ...
            if recurrent:
                ...
                action, log_prob, _, value = ppo.get_action(
...
class TestPPOTest:
    ...
    def test_algorithm_test_loop(
        self, observation_space, discrete_space, num_envs, request
    ):
        observation_space = request.getfixturevalue(observation_space)
    ...
    # Test PPO with hidden states and collect_rollouts
    def test_ppo_with_hidden_states_multiple_envs_collect_rollouts_and_test(self):
        num_envs = 4
        env = gymnasium.vector.SyncVectorEnv(
            [lambda: gymnasium.make("CartPole-v1")] * num_envs,
        )
    ...
    def test_ppo_test_loop_masks_callbacks_and_non_vectorized_paths(
        ...
        with pytest.warns(
            UserWarning,
            match="Action masks not provided for all vectorized environments",
        ):
            _ = ppo.test(
                VecEnv(),
                swap_channels=True,
...
class TestPPOClone:
    ...
    def test_clone_after_learning(
        ...
        if use_rollout_buffer:
            dummy_env = DummyEnv(
                observation_space.shape, vect=True, num_envs=num_vec_envs
            )
            # Use the correct rollout collection function based on whether the policy is recurrent
            collect_function = (
                collect_rollouts_recurrent if recurrent else collect_rollouts
            )
