...
class TestILQLForward:
    ...
    def test_forward_skip_policy_branch_train_vs_eval(self):
...
class TestILQLGetWeights:
    def test_get_weights_sample_raw(self):
        List_RL_Dataset.__abstractmethods__ = set()
        tokenizer = WordleTokenizer()
        token_reward = ConstantTokenReward(1)
        rl_ds = List_RL_Dataset(tokenizer, token_reward, 10)
...
class TestILQLGetLoss:
    ...
    def test_loss_helpers_return_scalar(self, double_q):
...
class TestILQLPrepareInputs:
    def test_prepare_inputs(self):
        ...
        meta = {"test": 1}
        dps = [
            DataPoint.from_obs(lang_obs, tokenizer, token_reward, meta)
            for _ in range(5)
        ]
...
class TestILQLHardUpdate:
    def test_hard_update(self):
        ...
        assert_state_dicts_equal(
            algo.actor_target.state_dict(), algo.model.state_dict()
        )
        algo.clean_up()
...
class TestILQLClone:
    def test_clone(self):
        ...
        assert_state_dicts_equal(
            algo.target_q.state_dict(), clone.target_q.state_dict()
        )
        assert_state_dicts_equal(algo.q2.state_dict(), clone.q2.state_dict())
        assert_state_dicts_equal(
            algo.target_q2.state_dict(), clone.target_q2.state_dict()
        )
...
class TestILQLSaveLoadCheckpoint:
    def test_save_load_checkpoint(self, tmpdir):
        List_RL_Dataset.__abstractmethods__ = set()
    ...
    def test_single_q_update_checkpoint_and_cleanup(self, tmpdir):
        algo = _make_algo(double_q=False)
...
class TestILQLScore:
    def test_score_no_logits_no_advantage_is_zero(self):
        algo = _make_algo(double_q=False)
        data = _simple_inputs()
    ...
    def test_score_trivial_value_query_with_action_mask(self):
        algo = _make_algo(double_q=False)
        tokens = torch.tensor([[0, 1, 2, 3, 1]])
...
class TestILQLClipValues:
    def test_clip_values_returns_unchanged_when_bounds_none(self):
        algo = _make_algo(double_q=False)
...
class TestILQLPolicyGenerate:
    def test_policy_generate_dispatch_and_sample_assert(self):
        algo = _make_algo(double_q=False)
        data = _simple_inputs()
...
class TestILQLEvaluatorEvaluate:
    def test_ilql_evaluator_evaluate_and_dump(self, monkeypatch):
        algo = _make_algo(double_q=False)
        data = _simple_inputs()
    ...
    def test_ilql_evaluator_verbose_and_top_advantage_ngram_branch(self, monkeypatch):
        algo = _make_algo(double_q=False)
        data = _simple_inputs()
...
class TestTopAdvantageNGramsEvaluate:
    def test_top_advantage_ngrams_evaluate_and_utils(self):
        class FakeData:
            def __init__(self):
                self.tokenizer = WordleTokenizer()
        ...
        evaluator = TopAdvantageNGrams(
            FakeData(), print_every=1, print_k=1, n_gram=None
        )
...
class TestILQLPolicyBeamRaw:
    def test_sample_and_beam_raw_cover_generation_loops(self, monkeypatch):
...
class TestILQLPolicyTrain:
    def test_policy_train_eval_and_invalid_kind_path(self):
        algo = _make_algo(double_q=False)
        policy = ILQL_Policy(algo, "beam")
        policy.train()
