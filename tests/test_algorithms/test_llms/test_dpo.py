...
class TestDPOLigerHiddenCapture:
    def test_get_hidden_never_runs_lm_head(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for attr in EvolvableAlgorithm.inspect_attributes(dpo):
                ...
                if attr == "rng":
                    ...
                    assert (
                        getattr(new_dpo, attr).lora_alpha
                        == getattr(dpo, attr).lora_alpha
                    )
                    assert (
                    ...
                    assert type(getattr(new_dpo, attr)) is type(getattr(dpo, attr))
                    assert (
...
class TestDPOBackwardPass:
    @pytest.mark.parametrize("batch_size", [1])
    def test_dpo_backward_pass(self, batch_size):
        vocab_size = 100
        input_size = 10
        max_tokens = 20
        dpo = _make_cpu_dpo_for_branch_tests()
...
class TestDPOPreprocessObservation:
    def test_dpo_preprocess_observation(self):
        dpo = _make_cpu_dpo_for_branch_tests()
...
class TestDPOSetReferencePolicy:
    @pytest.mark.parametrize("use_separate_reference_adapter", [False, True])
