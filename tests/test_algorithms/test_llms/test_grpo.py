...
class TestGRPOAdvantageGranularityDecoupling:
    """advantage granularity (action_granularity) is independent of the IS /
    ratio-pooling level (importance_sampling_level) for the GRPO family."""

    @staticmethod
    def _stub(action_granularity, importance_sampling_level):
    ...
    @pytest.mark.parametrize(
        "action_granularity,is_level,expected",
        [
    ...
    def test_resolve_advantage_granularity(
        self, action_granularity, is_level, expected
    ):
        stub = self._stub(action_granularity, is_level)
    ...
    def test_invalid_token_advantage_rejected(self):
        ...
        with pytest.raises(ValueError, match="action_granularity"):
            _make_cpu_grpo_for_branch_tests(action_granularity="token")
