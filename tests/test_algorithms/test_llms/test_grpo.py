...
def generate_grpo(
    ...
    if use_vllm:
        lora_config = None
        # Parallel vLLM workers on one GPU:
        #
        # ``kv_cache_memory_bytes`` pins the KV cache to a tiny fixed size and
        # skips vLLM's ``determine_available_memory`` profile-snapshot
        ...
        #
        # ``gpu_memory_utilization`` is not ignored when
        # ``kv_cache_memory_bytes`` is set. The startup check in
        ...
        # would fail with ``Free memory on device (X/14.58 GiB) on
...
class TestGRPOInit:
    ...
    def test_init_unset_mini_steps_every_micro_batch(self):
        ...
        assert grpo.batch_size == 4
        assert grpo.batch_size_per_process == 8
        assert grpo.mini_batch_size == 1
    ...
    def test_init_grpo_distributed(
        ...
        assert grpo.batch_size_per_process == 16 * group_size
    ...
    def test_init_grpo_vllm_with_tp_gt_one(
        ...
        mock_instance = make_mock_vllm_instance(vllm.LLM)
        dist_mode_factory(dist_mode)
        # Colocated vLLM rejects tensor parallelism: the in-process
        # shared base assumes a single worker, so TP>1 raises at construction.
        with (
            patch.object(
                torch.distributed,
    ...
    def test_init_grpo_batch_size_value_error(
        ...
        with (
            pytest.raises(
                ValueError,
                match=r"Batch size \(17\) must be divisible by the data-parallel size \(2\)\.",
            ),
            patch(
                "agilerl.algorithms.core.base.get_world_size",
                return_value=2,
    ...
    @pytest.mark.vllm
    @pytest.mark.parametrize("micro_batch_size_per_gpu", [None])
    def test_init_grpo_mini_batch_not_divisible(
        self,
        dist_mode_factory,
        model_factory,
        dist_mode,
        ...
        with (
            pytest.raises(
                ValueError,
                match=r"batch_size_per_process \(40\) must be divisible by mini_batch_size \(3\)",
            ),
            patch(
                "agilerl.algorithms.core.base.get_world_size",
                return_value=2,
    ...
    def test_init_grpo_micro_batch_size_per_gpu_division_error(
        ...
        with pytest.raises(
            ValueError,
            match=re.escape(
                f"batch_size_per_process ({batch_size * group_size}) must be "
                f"divisible by mini_batch_size ({micro_batch_size_per_gpu}): a rank "
...
class TestGRPOLigerSequencePacking:
    ...
    def test_packing_disabled_on_dense_backend_falls_back(self):
        ...
        lm_head = nn.Linear(hidden, vocab).to(grpo.device)
        grpo.use_sequence_packing = True
        grpo.actor.config._attn_implementation = "sdpa"
...
class TestGRPOLearn:
    ...
    def test_learn_multiprocess_all_filtered_masks_advantages_and_updates(self):
        ...
        assert seen_advantages
        assert all(torch.equal(adv, torch.zeros_like(adv)) for adv in seen_advantages)
        assert metrics["loss"] == 0.0
        assert metrics["kl"] == 0.0
        grpo.clean_up()
    ...
    def test_learn_raises_on_cross_rank_seq_len_mismatch(self):
        ...
        with (
            patch("agilerl.algorithms.grpo.get_world_size", return_value=2),
            patch(
                "agilerl.algorithms.grpo.needs_cross_rank_seq_padding",
                return_value=True,
            ),
            patch(
                "agilerl.algorithms.grpo.allreduce_minmax_int",
    ...
    def test_learn_all_filtered_joins_barrier_when_distributed(self):
        grpo = _make_cpu_grpo_for_branch_tests()
        completion_ids, action_masks = _build_branch_experiences(batch_size=2)
        rewards = torch.tensor([1.0, -1.0], dtype=torch.float32)
    ...
    def test_grpo_learn_raises_when_peer_rank_has_nonfinite_loss(self):
        ...
        with (
            ...
            patch(
                "agilerl.algorithms.grpo.allreduce_minmax_int", side_effect=fake_minmax
            ),
            patch.object(
                grpo, "_fused_forward_no_grad", side_effect=fake_fused_forward
            ),
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for attr in EvolvableAlgorithm.inspect_attributes(grpo):
                if not attr.startswith("_") and not attr.startswith("__"):
                    ...
                    elif attr == "shard_runtime":
                        assert type(getattr(new_grpo, attr)) is type(
                            getattr(grpo, attr)
                        )
                        assert (
...
class TestGRPOClone:
    ...
    def test_grpo_clone_distributed(
        ...
        grpo.fitness = [1, 2, 3]
        original_actor_state_dict = grpo.actor.state_dict()
...
class TestGRPOInitWarnings:
    def test_init_advantage_granularity_turn(self):
        grpo = _make_cpu_grpo_for_branch_tests(advantage_granularity="turn")
