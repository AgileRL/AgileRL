...
class TestMATD3Init:
    ...
    def test_parameter_sharing_group_networks_and_optimizers(self, ma_vector_space):
        ...
        assert set(matd3.actor_optimizers.optimizer.keys()) == set(
    ...
    def test_with_mlp_networks(
        ...
        assert all(
            isinstance(critic, expected_module_cls)
            for critic in matd3.critics_1.values()
        )
        assert all(
            isinstance(critic, expected_module_cls)
    ...
    def test_init_with_compile_no_error(self, mode, ma_vector_space, device):
        matd3 = MATD3(
            observation_spaces=ma_vector_space,
            action_spaces=copy.deepcopy(ma_vector_space),
            agent_ids=["agent_0", "agent_1", "other_agent_0"],
        ...
        if isinstance(mode, str):
            ...
            assert all(
                isinstance(m, OptimizedModule) for m in matd3.actor_targets.values()
            )
    ...
    def test_init_with_compile_error(self, mode, ma_vector_space, device):
        err_string = (
...
class TestMATD3Clone:
    ...
    def test_returns_identical_agent(
        ...
        for agent_id in clone_agent.agent_ids:
            network_id = get_network_id(matd3, agent_id)
            assert torch.equal(
                clone_agent.expl_noise[agent_id], matd3.expl_noise[agent_id]
            )
    ...
    @pytest.mark.parametrize("compile_mode", [None, "default"])
    def test_new_index(self, compile_mode, ma_vector_space, encoder_mlp_config):
        agent_ids = ["agent_0", "agent_1", "other_agent_0"]
