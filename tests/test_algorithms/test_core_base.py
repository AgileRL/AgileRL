...
class TestUnwrapModelsDictBranch:
    ...
    def test_wrap_models_wraps_dict_attributes(self, vector_space):
        ...
        agent.dummy_actor = {"agent_0": agent.dummy_actor}
        agent.wrap_models()
        assert isinstance(agent.dummy_actor, dict)
        assert "agent_0" in agent.dummy_actor
        agent.accelerator = None
...
def _make_llm_agent(
    ...
    agent.optimizer.state_dict.return_value = {}
    agent.optimizer.load_state_dict = MagicMock()
    inner = MagicMock()
    inner.param_groups = [{"lr": 1e-4, "params": [torch.tensor([1.0])]}]
...
class TestLLMAlgorithmLoad:
    ...
    def test_load_weights_raises_without_actor_state_dict(self, tmp_path):
        agent = _make_llm_agent()
        torch.save({"network_info": {}}, tmp_path / "attributes.pt")
...
class TestLLMAlgorithmPopulation:
    ...
    def test_population_builds_requested_size(self):
        with self._population_init_context():
            population = _StubLLMAlgorithm.population(
                size=3, device="cpu", **self._population_kwargs()
            )

        assert len(population) == 3
        assert [agent.index for agent in population] == [0, 1, 2]

    def test_population_copied_actors_use_clone_path(self):
        recorded: list[bool] = []

        class RecordingStub(_StubLLMAlgorithm):
            def __init__(self, *args, clone: bool, **kwargs):
                recorded.append(clone)
                super().__init__(*args, clone=clone, **kwargs)

        with self._population_init_context():
            population = RecordingStub.population(
                size=3, device="cpu", **self._population_kwargs()
            )

        assert len(population) == 3
        assert recorded == [False, True, True]

    def test_population_quantized_agents_clone(self):
        quant_config = SimpleNamespace(llm_int8_skip_modules=None)
        clones = [MagicMock(), MagicMock()]

        with (
            self._population_init_context(),
            patch.object(_StubLLMAlgorithm, "clone", side_effect=clones) as mock_clone,
        ):
            population = _StubLLMAlgorithm.population(
                size=3,
                device="cpu",
                **self._population_kwargs(quantization_config=quant_config),
            )

        assert len(population) == 3
        assert mock_clone.call_count == 2
        assert population[1] is clones[0]
        assert population[2] is clones[1]

    def test_population_resume_loads_each_agent(self):
        with (
            self._population_init_context(),
        ...
        mock_load.assert_called_with("/ckpt")
        assert [agent.index for agent in population] == [0, 1]

    def test_population_quantized_resume_loads_each_agent(self):
        quant_config = SimpleNamespace(llm_int8_skip_modules=None)
        clones = [MagicMock(), MagicMock()]

        with (
            self._population_init_context(),
            patch.object(_StubLLMAlgorithm, "clone", side_effect=clones),
            patch.object(_StubLLMAlgorithm, "load_checkpoint") as mock_load,
        ):
            population = _StubLLMAlgorithm.population(
                size=3,
        ...
        for clone, index in zip(clones, [1, 2], strict=True):
            clone.load_checkpoint.assert_called_once_with("/ckpt")
            assert clone.index == index
        assert population[0].index == 0


class TestLLMCloneHelpers:
    def test_resolve_clone_work_dir_broadcasts(self):
        agent = _make_llm_agent()

        assert agent._resolve_clone_work_dir("/tmp/work") == "/tmp/work"

    def test_save_clone_adapter_weights(self, tmp_path):
        agent = _make_llm_agent()
        agent.selected_adapters = ["actor"]

        with (
            patch("agilerl.algorithms.core.base.save_lora_adapters") as mock_save,
            patch("agilerl.algorithms.core.base.barrier") as mock_barrier,
        ):
            agent._save_clone_adapter_weights(str(tmp_path))

        mock_save.assert_called_once_with(
            model=agent.actor,
            path=f"{tmp_path}/adapters",
    ...
    def test_load_clone_adapter_weights(self, tmp_path):
        agent = _make_llm_agent()
        agent.selected_adapters = ["actor"]

        with patch.object(agent, "_load_adapter_weights") as mock_load:
            agent._load_clone_adapter_weights(str(tmp_path))

        mock_load.assert_called_once_with(f"{tmp_path}/adapters", "actor")

    def test_create_clone_instance_quantized_rebuilds_from_pretrained(self):
        agent = _make_llm_agent()
        agent.quantization_config = MagicMock()
        agent.pretrained_model_name_or_path = "stub/model"

        with (
            patch.object(_StubLLMAlgorithm, "__init__", return_value=None) as mock_init,
            patch.object(EvolvableAlgorithm, "_registry_init"),
        ):
            clone = agent._create_clone_instance()

        assert mock_init.call_args.kwargs["actor_network"] is None
        assert mock_init.call_args.kwargs["model_name"] == "stub/model"
        assert isinstance(clone, _StubLLMAlgorithm)

    def test_clone_quantized_copies_value_head(self):
        agent = _make_llm_agent()
        agent.quantization_config = MagicMock()
        agent.use_value_head = True
        agent.actor = SimpleNamespace(v_head=nn.Linear(2, 2))
        torch.nn.init.ones_(agent.actor.v_head.weight)
        clone = SimpleNamespace(
            actor=SimpleNamespace(v_head=nn.Linear(2, 2)),
            mutation_hook=MagicMock(),
            wrap_models=MagicMock(),
            _load_clone_adapter_weights=MagicMock(),
        )

        with (
            patch.object(
                agent,
                "_resolve_clone_work_dir",
                return_value="/tmp/work",
            ),
            patch.object(agent, "_save_clone_adapter_weights"),
            patch.object(agent, "_create_clone_instance", return_value=clone),
            patch.object(agent, "_copy_clone_attributes", side_effect=lambda c: c),
            patch.object(agent, "_restore_clone_optimizer_and_scheduler"),
        ):
            out = agent.clone()

        assert out is clone
        assert torch.equal(clone.actor.v_head.weight.data, torch.ones(2, 2))

    def test_copy_clone_attributes_transfers_sleep_engine(self):
        agent = _make_llm_agent()
        agent.colocated = True
        agent.vllm_config = SimpleNamespace(sleep_mode=True)
        engine = object()
        agent.llm = engine
        agent._vllm_awake = True
        agent._vllm_lora_staging_dir = "/tmp/stage"
        agent._vllm_lora_loaded = True
        agent._vllm_rollout_lora_request = MagicMock()
        clone = SimpleNamespace()

        out = agent._copy_clone_attributes(clone)

        assert out.llm is engine
        assert out._vllm_awake is True
        assert agent.llm is None
        assert agent._vllm_lora_staging_dir is None
        assert agent._vllm_lora_loaded is False
        assert agent._vllm_rollout_lora_request is None


class TestAttachLoraAdaptersExperts:
    def test_expert_lora_upgrades_wrappers_and_unifies_dtypes(self):
        from peft import LoraConfig

        agent = _make_llm_agent()
        agent.lora_config = LoraConfig(
            r=2, target_parameters=["experts.gate_up_proj"], lora_dropout=0.0
        )
        agent.selected_adapters = ["actor"]
        agent.shard_runtime = FSDPRuntime(FSDPConfig())
        base_model = nn.Module()
        base_model.experts = nn.Module()
        base_model.experts.gate_up_proj = nn.Parameter(torch.ones(2, 2))
        base_weight = nn.Parameter(torch.ones(2, 2))
        lora_weight = nn.Parameter(torch.ones(2, 2, dtype=torch.float16))
        peft_model = SimpleNamespace(
            peft_config={"actor": MagicMock()},
            named_parameters=lambda: [
                ("base.weight", base_weight),
                ("lora_A.weight", lora_weight),
            ],
            add_adapter=MagicMock(),
        ...
        with (
            ...
            patch(
                ...
                return_value=agent.lora_config,
            ),
            patch(
            ...
            out = agent._attach_lora_adapters(base_model)

        assert out is peft_model
        mock_upgrade.assert_called_once_with(peft_model)
        assert lora_weight.dtype == torch.float32


class TestInitializeActorsCompiler:
    def test_compiler_disables_activation_offload(self):
        agent = _make_llm_agent()
        agent.torch_compiler = "default"
        agent.gradient_checkpointing = False
        agent.activation_offload = True
        actor = MagicMock()

        with (
            patch.object(agent, "_attach_lora_adapters", return_value=actor),
            patch.object(agent, "use_adapter"),
            patch("agilerl.algorithms.core.base.patch_lora_for_fused_forward"),
            patch("agilerl.algorithms.core.base.install_packed_expert_grouped_gemm"),
            patch(
                "agilerl.algorithms.core.base.compile_model", return_value=actor
            ) as mock_compile,
            pytest.warns(UserWarning, match="activation offload"),
        ):
            agent._initialize_actors(MagicMock())

        assert agent.activation_offload is False
        mock_compile.assert_called_once_with(actor, "default")
...
class TestConfigureBatchSizePerProcess:
    def test_batch_default_accumulates_the_rank_batch(self):
        ...
        agent.configure_batch_size_per_process(4, 1, None)

        assert agent.mini_batch_size == 4
        assert agent.gradient_accumulation_steps == 4

    def test_micro_batch_default_steps_every_micro_batch(self):
        agent = _make_llm_agent()
        agent._mini_batch_size_default = "micro_batch"

        agent.configure_batch_size_per_process(4, 1, None)

        assert agent.mini_batch_size == 1
        assert agent.gradient_accumulation_steps == 1

    def test_micro_batch_default_without_micro_uses_the_rank_batch(self):
        agent = _make_llm_agent()
        agent._mini_batch_size_default = "micro_batch"

        agent.configure_batch_size_per_process(4, None, None)

        assert agent.mini_batch_size == 4
        assert agent.micro_batch_size_per_gpu == 4

    def test_rejects_non_positive_mini_batch_size(self):
        agent = _make_llm_agent()

        with pytest.raises(ValueError, match="mini_batch_size must be a positive"):
            agent.configure_batch_size_per_process(4, 2, 0)

    def test_rejects_non_positive_micro_batch_size(self):
        agent = _make_llm_agent()

        with pytest.raises(
            ValueError, match="micro_batch_size_per_gpu must be a positive"
        ):
            agent.configure_batch_size_per_process(4, -1, 4)
...
class TestLLMDistributedValidation:
    ...
    def test_fsdp_config_disables_nccl_cumem(self, monkeypatch):
...
class TestFusedLinearLogprobsIntegration:
    ...
    def _build_lora_agent(self, vocab_size: int, hidden_size: int) -> LLMAlgorithm:
        ...
        model = inject_adapter_in_model(
            ...
            _TinyCausalLM(vocab_size, hidden_size),
            adapter_name="actor",
        )
        patch_lora_for_fused_forward(model)
        model.name_or_path = "tiny-causal"
        model.eval()
        agent = _make_llm_agent(actor_network=model)
        agent.use_value_head = False
        agent.calc_position_embeddings = False
        agent.pad_token_id = 0
    ...
    def _per_row_logprobs(
        ...
        rows = [
            ...
            for i, name in enumerate(routing)
        ]
    ...
    def test_fused_model_pass_gathers_lm_head_once_across_chunks(self) -> None:
        from contextlib import contextmanager

        torch.manual_seed(5)
        B, T, H, V = 2, 4, 8, 64
        agent, _ = self._build_agent(V, H)
        fused_ids = torch.randint(1, V, (2 * B, T))
        fused_mask = torch.ones_like(fused_ids)
        routing = ["__base__"] * B + ["actor"] * B
        original = agent.shard_runtime.gather_layer
        calls = {"n": 0}

        @contextmanager
        def counting_gather(layer, device):
            calls["n"] += 1
            with original(layer, device=device) as pair:
                yield pair
        ...
        with (
            ...
            patch.object(agent.shard_runtime, "gather_layer", counting_gather),
            torch.no_grad(),
        ):
            log_probs, values = agent._fused_model_pass(
                fused_ids, fused_mask, routing, batch_size=1
            )

        assert calls["n"] == 1
        assert log_probs.shape == (2 * B, T - 1)
        assert values is None

    def test_fused_model_pass_spans_straddle_adapter_runs(self) -> None:
        torch.manual_seed(7)
        B, T, H, V = 2, 4, 8, 64
        agent, _ = self._build_agent(V, H)
        fused_ids = torch.randint(1, V, (2 * B, T))
        ...
        with (
            ...
            patch.object(agent, "_fused_chunk_hidden_and_value", counting_hidden),
            torch.no_grad(),
        ):
            log_probs, _ = agent._fused_model_pass(
                fused_ids, fused_mask, routing, batch_size=3
            )

        assert hidden_calls == [3, 1]
        assert log_probs.shape == (2 * B, T - 1)

    @pytest.mark.parametrize("batch_size", [1, 3])
    def test_fused_model_pass_span_logprobs_match_per_row(self, batch_size) -> None:
        # Arrange
        B, T, H, V = 3, 6, 12, 48
        agent = self._build_lora_agent(V, H)
        torch.manual_seed(2)
        fused_ids = torch.randint(1, V, (2 * B, T))
        fused_ids[0] = 1
        fused_ids[-1] = V - 1
        ...
        # Act
        with torch.no_grad():
            per_row = self._per_row_logprobs(agent, fused_ids, fused_mask, routing)
            chunked, _ = agent._fused_model_pass(
...
class TestLLMBackwardPassGradClipping:
    def test_backward_pass_calls_clip_grad_norm(self):
        agent = _make_llm_agent()
        agent.max_grad_norm = 1.0
        agent.micro_batch_size_per_gpu = 1
        agent.gradient_accumulation_steps = 1
        param = nn.Parameter(torch.tensor([1.0]))
        opt = torch.optim.SGD([param], lr=0.1)
        agent.optimizer = MagicMock()
        agent.optimizer._single_optimizer.return_value = opt
        agent.shard_runtime = DPRuntime()
        loss = (param * 2).sum()
        with patch("agilerl.distributed.runtime.clip_grad_norm_") as mock_clip:
...
class TestLLMBackwardPassGradNorms:
    @staticmethod
    def _agent_and_loss(grad, max_grad_norm, accumulation_steps=1):
        agent = _make_llm_agent()
        agent.max_grad_norm = max_grad_norm
        agent.gradient_accumulation_steps = accumulation_steps
        agent.lr_scheduler = None
        param = nn.Parameter(torch.zeros(len(grad)))
        agent.optimizer = MagicMock()
        agent.optimizer._single_optimizer.return_value = torch.optim.SGD(
            [param], lr=0.0
        )
        agent.shard_runtime = DPRuntime()
        return agent, (param * torch.tensor(grad)).sum()
...
class TestLLMBackwardPassDistributed:
    ...
    def test_backward_pass_syncs_replicated_grads_with_fsdp(self):
        """FSDP2 reduce-scatters DTensors; replicated LoRA still needs all-reduce."""
        with patch("agilerl.algorithms.core.base.init_distributed", return_value=True):
            agent = _make_llm_agent(fsdp_config=FSDPConfig(defer_grad_sync=False))
        agent.max_grad_norm = None
        agent.lr_scheduler = None
...
class TestLLMClonePlain:
    ...
    def test_clone_copies_optimizer_state(self):
        ...
        cloned.shard_runtime.import_optimizer_state.assert_called_once()
        cloned.wrap_models.assert_called_once()

    def test_copy_clone_attributes_keeps_clone_actor_and_shard_runtime(self):
        agent = _make_llm_agent(clone=True)
        agent.colocated = False
        clone = _make_llm_agent(clone=True)
        clone.colocated = False
        clone_actor = object()
        clone_runtime = object()
        clone.actor = clone_actor
        clone.shard_runtime = clone_runtime
        agent.actor = object()
        agent.shard_runtime = object()

        out = LLMAlgorithm._copy_clone_attributes(agent, clone)

        assert out.actor is clone_actor
        assert out.shard_runtime is clone_runtime
...
class TestLLMCloneActorNetwork:
    ...
    def _make_value_head_actor():
        class _StubValueHead:
            ...
            def state_dict(self):
                inner_sd = self.pretrained_model.state_dict()
                return {
                    **{f"pretrained_model.{k}": v for k, v in inner_sd.items()},
                    **{f"v_head.{k}": v for k, v in self.v_head.state_dict().items()},
                }
    ...
    def test_value_head_fsdp_clone_exports_actor_root(self):
        agent = _make_llm_agent()
        agent.use_value_head = True
        agent.actor = self._make_value_head_actor()
        original_v_head_weight = agent.actor.v_head.weight.detach().clone()
        full_state = {
            "pretrained_model.w": torch.tensor([3.0, 4.0]),
            "v_head.weight": original_v_head_weight,
            "v_head.bias": agent.actor.v_head.bias.detach().clone(),
        }
        cloned_inner = MagicMock()

        with (
            patch.object(
                agent.shard_runtime, "export_model_state", return_value=full_state
            ) as mock_export,
            patch.object(agent, "_broadcast_cpu_state_dict", return_value=full_state),
            patch(
                "agilerl.algorithms.core.base.clone_llm", return_value=cloned_inner
            ) as mock_clone,
        ):
        ...
        mock_export.assert_called_once_with(agent.actor)
        mock_clone.assert_called_once()
        assert mock_clone.call_args.kwargs["state_dict"] == {
            "w": full_state["pretrained_model.w"]
        }
        assert cloned.pretrained_model is cloned_inner
        assert torch.equal(cloned.v_head.weight, original_v_head_weight)
...
class TestLLMMemoryEfficientParams:
    """_offload_trainer_for_rollout context manager moves params on/off GPU when
    ...
    def test_sharded_actor_warns_and_yields_without_moving_params(self):
        ...
        with (
            ...
            pytest.warns(
                UserWarning,
                match="Trainer offload during rollout is not compatible with FSDP2",
            ),
            agent._offload_trainer_for_rollout(),
        ):
            pass
    ...
    def test_default_path_moves_params_to_gpu_and_back_to_cpu(self):
        ...
        with (
            ...
            with agent._offload_trainer_for_rollout():
...
class TestOffloadTrainerToCpuForColocatedVllm:
    def test_warns_when_actor_is_missing(self):
        agent = _make_llm_agent()
        agent.actor = None
        with pytest.warns(UserWarning, match="actor not set"):
            LLMAlgorithm._offload_trainer_to_cpu_for_colocated_vllm(agent)

    def test_offloads_and_warns_when_cuda_bytes_remain(self):
        agent = _make_llm_agent()
        agent.actor = MagicMock()
        with (
            patch(
            ...
            patch(
                "agilerl.algorithms.core.base.is_main_process",
                return_value=True,
            ),
            patch("agilerl.algorithms.core.base.log_cuda_memory_snapshot") as log,
            pytest.warns(UserWarning, match="still has"),
        ):
            LLMAlgorithm._offload_trainer_to_cpu_for_colocated_vllm(agent)
        assert log.call_count == 2

    def test_offloads_without_leftover_warning_on_non_main(self):
        agent = _make_llm_agent()
        agent.actor = MagicMock()
        with (
            patch(
            ...
            LLMAlgorithm._offload_trainer_to_cpu_for_colocated_vllm(agent)
        offload.assert_called_once_with(agent.actor)
...
class TestLLMConfigureVllmPaths:
    ...
    def test_configure_vllm_tp_size_gt_1(self, distributed_env):
        ...
        try:
            ...
            # _make_llm_agent initialises a real group from distributed_env;
            # destroy it so later tests see a single device.
            if (
                not had_group
...
class TestLLMShardedCheckpointBranches:
    ...
    def test_export_optimizer_state_sharded_uses_dcp(self):
        ...
        with (
            ...
            out = agent.shard_runtime.export_optimizer_state(
                agent.actor, agent.optimizer
            )
...
class TestLLMFusedForwardPaths:
    ...
    def test_fused_forward_no_grad_respects_microbatch_size(self):
        ...
        agent._fused_model_pass = MagicMock(
            return_value=(torch.zeros(2 * B, T - 1), None)
        )

        agent._fused_forward_no_grad(ids, batch_size=1)

        kwargs = agent._fused_model_pass.call_args.kwargs
        fused_ids = agent._fused_model_pass.call_args.args[0]
        routing = agent._fused_model_pass.call_args.args[2]
        assert fused_ids.shape[0] == 2 * B
...
class TestLoraInputCastUnderAutocast:
    ...
    @staticmethod
    def _agent(actor):
        agent = SimpleNamespace(actor=actor, device="cuda", distributed=False)
