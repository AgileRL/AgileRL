...
class TestLLMDistributedValidation:
    ...
    def test_fsdp_config_disables_nccl_cumem(self, monkeypatch):
        ...
        monkeypatch.delenv("NCCL_TUNER_PLUGIN", raising=False)
        ...
        assert os.environ["NCCL_TUNER_PLUGIN"] == "none"
        assert os.environ["NCCL_NET_PLUGIN"] == "none"
        assert os.environ["NCCL_PROFILER_PLUGIN"] == "none"
...
class TestFusedLinearLogprobsIntegration:
    ...
    def test_fused_model_pass_mixes_adapters_per_sequence(self) -> None:
        torch.manual_seed(6)
        B, T, H, V = 2, 4, 8, 64
        agent, _ = self._build_agent(V, H)
        fused_ids = torch.randint(1, V, (2 * B, T))
        ...
        assert hidden_calls == [2, 2]
        assert log_probs.shape == (2 * B, T - 1)

    def test_fused_model_pass_keeps_adapter_aligned_chunks_for_packed_experts(
        self,
    ) -> None:
        torch.manual_seed(7)
        B, T, H, V = 2, 4, 8, 64
        ...
        with (
            ...
            patch(
                "agilerl.algorithms.core.base._is_packed_experts_module",
                return_value=True,
            ),
            patch.object(agent, "_fused_chunk_hidden_and_value", counting_hidden),
            torch.no_grad(),
        ):
            agent._fused_model_pass(fused_ids, fused_mask, routing, batch_size=1)

        assert hidden_calls == [1, 1, 1, 1]

    def test_fused_model_pass_mix_logprobs_match_adapter_aligned(self) -> None:
        B, T, H, V = 2, 8, 16, 64
        ...
        def counting_hidden(bucket: list[int], chunk_ids, chunk_mask, chunk_pos):
            bucket.append(int(chunk_ids.shape[0]))
            return orig_hidden(chunk_ids, chunk_mask, chunk_pos)
        ...
        with torch.no_grad():
            ...
            with patch.object(
                agent,
                "_fused_chunk_hidden_and_value",
                lambda ids, mask, pos: counting_hidden(mix_widths, ids, mask, pos),
            ):
                mixed, _ = agent._fused_model_pass(
                    fused_ids, fused_mask, routing, batch_size=1
                )
            with (
                patch(
                ...
                patch.object(
                    agent,
                    "_fused_chunk_hidden_and_value",
                    lambda ids, mask, pos: counting_hidden(seq_widths, ids, mask, pos),
                ),
            ):
                sequential, _ = agent._fused_model_pass(
                    fused_ids, fused_mask, routing, batch_size=1
                )

        assert mixed.shape == sequential.shape == per_row.shape == (2 * B, T - 1)
        assert mix_widths == [2, 2]
        assert seq_widths == [1, 1, 1, 1]
        assert not torch.allclose(mixed[:B], mixed[B:], atol=1e-4)
        assert torch.allclose(mixed, sequential, rtol=1e-5, atol=1e-5)
        assert torch.allclose(mixed, per_row, rtol=1e-5, atol=1e-5)
        assert torch.allclose(sequential, per_row, rtol=1e-5, atol=1e-5)

    def test_fused_model_pass_mix_logprobs_match_when_sequence_ids_differ(
        self,
    ) -> None:
        B, T, H, V = 3, 6, 12, 48
        agent = self._build_lora_agent(V, H)
...
class TestLLMCloneActorNetwork:
    ...
    def test_value_head_fsdp_clone_exports_actor_root(self):
        ...
        mock_export.assert_called_once_with(agent.actor, cpu_offload=True)
