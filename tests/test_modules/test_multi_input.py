...
class TestEvolvableMultiInputAddMlpLayer:
    ...
    def test_add_mlp_layer_else_statement(self, device, default_mlp_config):
        config = default_mlp_config.copy()
...
class TestEvolvableMultiInputAddMlpNode:
    ...
    def test_add_nodes(
        ...
        original_hidden_size = copy.deepcopy(
            evolvable_composed.mlp_config["hidden_size"]
        )
        layer = layer_index
...
class TestEvolvableMultiInputActivation:
    def test_multi_input_activation_setter(
        self, device, default_cnn_config, default_mlp_config
    ):
...
class TestEvolvableMultiInputInitWeightsGaussian:
...
class TestEvolvableMultiInputChangeActivation:
...
class TestEvolvableMultiInputAddCnnLayer:
    ...
    def test_add_cnn_layer_else_statement(self, device, default_cnn_config):
        config = default_cnn_config.copy()
...
class TestEvolvableMultiInputAddChannel:
    ...
    def test_add_channels(
        ...
        original_channel_size = copy.deepcopy(
            evolvable_composed.cnn_config["channel_size"]
        )
...
class TestEvolvableMultiInputChangeKernel:
    def test_change_cnn_kernel(self, device, default_cnn_config, dict_space):
        config = default_cnn_config.copy()
        config["channel_size"] = [32, 32]
        config["kernel_size"] = [3, 3]
        config["stride_size"] = [1, 1]
        ...
        while evolvable_composed.init_dicts["image_1"]["kernel_size"] == [
            (3, 3),
            (3, 3),
        ]:
            getattr(evolvable_composed, "feature_net.image_1.change_kernel")()
    ...
    def test_change_kernel_size(self, device, default_cnn_config, dict_space):
        config = default_cnn_config.copy()
        config["channel_size"] = [32, 32]
        config["kernel_size"] = [3, 3]
        config["stride_size"] = [1, 1]
    ...
    def test_change_cnn_kernel_else_statement(
        self, device, default_cnn_config, dict_space
    ):
        config = default_cnn_config.copy()
        config["channel_size"] = [32, 32]
        config["kernel_size"] = [3, 3]
    ...
    def test_change_cnn_kernel_multi(self, device, multiagent_cnn_config, dict_space):
