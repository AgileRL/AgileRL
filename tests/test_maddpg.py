...
def test_getAction_epsilon_greedy_mlp(
...
def test_getAction_epsilon_greedy_cnn(
...
def test_getAction_epsilon_greedy_distributed_cnn(
    epsilon, state_dims, action_dims, discrete_actions
):
    accelerator = Accelerator()
    agent_ids = ["agent_0", "agent_1"]
...
def test_getAction_agent_masking(
    epsilon, state_dims, action_dims, discrete_actions, device
):
    agent_ids = ["agent_0", "agent_1"]
    state = {agent: np.random.randn(*state_dims[0]) for agent in agent_ids}
...
def test_learns_from_experiences_mlp(
    state_dims,
    experiences,
    discrete_actions,
    batch_size,
...
def test_learns_from_experiences_mlp_distributed(
    state_dims,
...
def test_learns_from_experiences_cnn(
    state_dims,
    experiences,
    discrete_actions,
    batch_size,
...
def test_learns_from_experiences_cnn_distributed(
    state_dims,
    experiences,
    discrete_actions,
    batch_size,
...
def test_algorithm_test_loop(device):
    state_dims = [(6,), (6,)]
    action_dims = [2, 2]
    accelerator = None
...
def test_algorithm_test_loop_cnn(device):
...
def test_clone_returns_identical_agent(accelerator_flag, wrap):
...
def test_save_load_checkpoint_correct_data_and_format_make_evo(
