...
class RolloutHarnessDouble(RolloutHarness):
    ...
    Algorithm tests that stub ``reset`` / ``step`` still have to be a
    ``RolloutHarness`` (``LLMAlgorithm.test`` checks ``isinstance``). This
    runs the real initializer so instance state exists.
    """
    ...
    def __init__(self) -> None:
        super().__init__(
            FakeEnvClient(),
            MiniTokenizer(),
            max_turns=getattr(type(self), "max_turns", 1),
