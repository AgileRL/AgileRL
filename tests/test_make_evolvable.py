...
######### Test get_activation #########
def test_returns_correct_activation_function_for_all_supported_names(simple_mlp,device):
    activation_names = [
        "Tanh",
        "ReLU",
        "ELU",
    ...
    for name in activation_names:
        activation = MakeEvolvable(simple_mlp, torch.randn(1,10), device=device).get_activation(name)
...
def test_forward_method(
    ...
    if secondary_input_tensor is None:
        ...
        with torch.no_grad():
            ...
            actual_output = evolvable_network.forward(input_tensor, secondary_input_tensor)
...
def test_detect_architecture_mlp_simple(device):
    ...
    evolvable_net = MakeEvolvable(net, torch.randn(1, 4), device=device, output_vanish=True)
...
def test_detect_architecure_exception(device):
    net = nn.Sequential(
        nn.Linear(4, 16), nn.Linear(16, 16), nn.ReLU(), nn.Linear(16, 1), nn.LogSoftmax(dim=-1)
    )
...
def test_detect_architecture_empty_hidden_size(device):
    ...
    with pytest.raises(TypeError):
        evolvable_net = MakeEvolvable(net, torch.randn(1, 2), device=device)
...
def test_add_mlp_layer_simple(simple_mlp, device):
    input_tensor = torch.randn(1, 10)
    evolvable_network = MakeEvolvable(simple_mlp, input_tensor, init_layers=True, device=device)
...
def test_add_mlp_layer_else_statement(device):
    ...
    evolvable_network = MakeEvolvable(network, torch.randn(1, 4), device=device, max_hidden_layers=2)
    initial_num_layers = len(evolvable_network.hidden_size)
    evolvable_network.add_mlp_layer()
...
def test_remove_mlp_layer_else_statement(device):
    ...
    evolvable_network = MakeEvolvable(network, torch.randn(1, 4), device=device, min_hidden_layers=2)
    initial_num_layers = len(evolvable_network.hidden_size)
    evolvable_network.remove_mlp_layer()
    assert initial_num_layers == len(evolvable_network.hidden_size)
...
def test_remove_mlp_layer_no_output_activation(device):
    ...
    initial_num_layers = len(evolvable_network.hidden_size)
    assert evolvable_network.mlp_layer_info == {
        "activation_layers": {0: "ReLU", 2: "Tanh"}},evolvable_network.mlp_layer_info
    evolvable_network.remove_mlp_layer()
    new_value_net = evolvable_network.value_net
    assert len(evolvable_network.hidden_size) == initial_num_layers - 1
    assert evolvable_network.mlp_layer_info == {
        "activation_layers": {0: "ReLU", 1: "Tanh"}}, evolvable_network.mlp_layer_info
    for key, param in new_value_net.named_parameters():
        if key in value_net_dict.keys():
            assert torch.equal(param, value_net_dict[key])
...
def test_remove_mlp_node_no_arg(device):
    ...
    # Check if the hidden layer and number of new nodes are updated correctly
    assert evolvable_network.hidden_size[hidden_layer] == original_hidden_size[hidden_layer] - numb_new_nodes
...
def test_add_cnn_layer_multi(two_arg_cnn, device):
    evolvable_cnn  = MakeEvolvable(two_arg_cnn, 
...
def test_add_cnn_layer_else_statement(simple_cnn, device):
    evolvable_cnn = MakeEvolvable(simple_cnn, torch.randn(1, 3, 64, 64), device=device, max_cnn_hidden_layers=1)
    original_channels = copy.deepcopy(evolvable_cnn.channel_size)
    evolvable_cnn.add_cnn_layer()
...
def test_remove_cnn_layer_else_statement(simple_cnn, device):
    evolvable_cnn = MakeEvolvable(simple_cnn, torch.randn(1, 3, 64, 64), device=device, min_cnn_hidden_layers=2)
    original_channels = copy.deepcopy(evolvable_cnn.channel_size)
    evolvable_cnn.remove_cnn_layer()
    assert len(original_channels) == len(evolvable_cnn.channel_size)   
...
def test_remove_cnn_channel(device):
    input_tensor = torch.randn(1, 3, 64, 64)
    cnn = nn.Sequential(nn.Conv2d(3, 256, 3, 2), nn.ReLU(), nn.Conv2d(256,128,3,2), nn.ReLU(), 
                        nn.Flatten(), nn.Linear(28800, 128), nn.ReLU(), nn.Linear(128,10))
    ...
    result = evolvable_network.remove_cnn_channel(hidden_layer=layer, numb_new_channels=numb_new_channels)
    numb_new_channels = result["numb_new_channels"]
    hidden_layer = result["hidden_layer"]
    assert evolvable_network.channel_size[hidden_layer] == original_channel_size[hidden_layer] - numb_new_channels
...
def test_remove_cnn_channel_specified_hidden_layer(device):
    ...
    result = evolvable_network.remove_cnn_channel(hidden_layer=layer, numb_new_channels=numb_new_channels)
    numb_new_channels = result["numb_new_channels"]
    assert evolvable_network.channel_size[layer] == original_channel_size[layer] - numb_new_channels
...
def test_change_kernel_multi_single_arg(device):
    ...
    evolvable_cnn = MakeEvolvable(network, 
...
def test_clone_method_with_equal_state_dicts(
    ...
    network = request.getfixturevalue(network)
    if secondary_input_tensor is None:
        evolvable_network = MakeEvolvable(network,
                                          input_tensor, 
                                          device=device, 
