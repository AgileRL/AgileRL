...
class TestSaveLlmCheckpoint:
    def test_save_without_distributed(self, tmp_path):
        """Checkpoint save uses barrier + save_checkpoint, no Accelerator."""
        agent = Mock()
        agent.actor = Mock()
        agent.algo = "grpo"
...
class TestConsolidateMutations:
    ...
    def test_consolidate_mutations(self):
        ...
        with patch(
            ...
            side_effect=lambda objects, src=0: objects,
        ):
            consolidate_mutations(population)
        for agent in population:
            assert agent.mut == "lr"
...
class TestPrepareLlmAlgoKwargs:
    ...
    def test_lora_target_scope_and_chunk_rows_merge_from_init_hp(self):
        from agilerl.utils.utils import _prepare_llm_algo_kwargs
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(LORA_TARGET_SCOPE="all", CHUNK_ROWS=4),
        )

        assert merged["lora_target_scope"] == "all"
        assert merged["chunk_rows"] == 4
    ...
    def test_fsdp_true_builds_default_config(self):
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(FSDP=True),
        )

        assert isinstance(merged["fsdp_config"], FSDPConfig)

    def test_fsdp_empty_dict_builds_default_config(self):
        from agilerl.distributed import FSDPConfig
        from agilerl.utils.utils import _prepare_llm_algo_kwargs
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(FSDP={}),
        )

        assert isinstance(merged["fsdp_config"], FSDPConfig)

    def test_fsdp_dict_builds_config(self):
        from agilerl.utils.utils import _prepare_llm_algo_kwargs
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(FSDP={"cpu_offload": True}),
        )

        assert merged["fsdp_config"].cpu_offload is True

    def test_fsdp_config_instance_is_passed_through(self):
        from agilerl.distributed import FSDPConfig
        from agilerl.utils.utils import _prepare_llm_algo_kwargs
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(FSDP=config),
        )

        assert merged["fsdp_config"] is config

    def test_fsdp_false_leaves_config_unset(self):
        from agilerl.utils.utils import _prepare_llm_algo_kwargs
        ...
        merged = _prepare_llm_algo_kwargs(
            ...
            lora_config=None,
            vllm_config=None,
            INIT_HP=self._init_hp(FSDP=False),
        )

        assert "fsdp_config" not in merged

    @pytest.mark.parametrize("junk", ["true", 1, "{}"])
    def test_fsdp_truthy_junk_raises(self, junk):
        from agilerl.utils.utils import _prepare_llm_algo_kwargs

        with pytest.raises(TypeError, match=r"INIT_HP\['FSDP'\]"):
            _prepare_llm_algo_kwargs(
                ...
                lora_config=None,
                vllm_config=None,
                INIT_HP=self._init_hp(FSDP=junk),
            )
...
class TestDataParallelTopology:
    def test_identity_on_a_single_process(self):
        from agilerl.utils.utils import data_parallel_topology

        # Act / Assert
        assert data_parallel_topology() == (0, 1)

    def test_identity_when_unsharded(self):
        from agilerl.utils.utils import data_parallel_topology

        with (
            patch("agilerl.utils.utils.get_world_size", return_value=4),
            patch("agilerl.utils.utils.get_rank", return_value=2),
        ):
            # Act / Assert
            assert data_parallel_topology(1) == (2, 4)
    ...
    def test_tensor_parallel_ranks_share_one_replica_index(
        ...
        with (
            patch("agilerl.utils.utils.get_world_size", return_value=8),
            patch("agilerl.utils.utils.get_rank", return_value=process_index),
        ):
            # Act / Assert
            assert data_parallel_topology(2) == (expected_rank, 4)

    def test_a_replica_straddling_the_process_group_is_rejected(self):
        from agilerl.utils.utils import data_parallel_topology

        with (
            patch("agilerl.utils.utils.get_world_size", return_value=6),
            patch("agilerl.utils.utils.get_rank", return_value=0),
            pytest.raises(ValueError, match="does not divide"),
        ):
            # Act
            data_parallel_topology(4)
