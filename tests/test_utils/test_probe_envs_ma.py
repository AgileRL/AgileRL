...
class TestCheckPolicyQLearningWithProbeEnv:
    ...
    def test_policy_q_learning_with_probe_env(self):
        ...
        algo_args = {
            "observation_spaces": [
                env.observation_space[agent] for agent in env.agents
            ],
    ...
    @pytest.mark.gpu
    def test_policy_q_learning_with_probe_env_mlp(self):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
class TestCheckOnPolicyLearningWithProbeEnv:
    ...
    def test_on_policy_learning_with_probe_env_mlp(self, Env, discrete):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        env = Env()
        learn_steps = 20
        algo_args = {
