...
class TestApplyFsdp2:
    ...
    def test_checkpoint_wraps_blocks_then_shards_the_wrapper(self, world_size_one):
        ...
        def _capture(_model, block_units, prefetch_count=2):
    ...
    def test_prefetch_units_one_is_neighbour_only(self, world_size_one):
        class PrefetchBlock(nn.Module):
            def __init__(self):
                super().__init__()
                self.lin = nn.Linear(2, 2)
        ...
        with (
            ...
            apply_fsdp2(model, FSDPConfig(prefetch_units=1, wrap_every_n_blocks=1))
    ...
    def test_rejects_prefetch_units_below_one(self, world_size_one):
        with pytest.raises(ValueError, match="prefetch_units must be >= 1"):
            apply_fsdp2(nn.Linear(2, 2), FSDPConfig(prefetch_units=0))

    def test_rejects_negative_param_persistence_threshold(self, world_size_one):
        with pytest.raises(ValueError, match="param_persistence_threshold must be >= 0"):
            apply_fsdp2(nn.Linear(2, 2), FSDPConfig(param_persistence_threshold=-1))
