...
class TestAsyncPettingZooVecEnvInit:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_create_async_pz_vector_env(self, env_fns):
...
class TestAsyncPettingZooVecEnvRender:
    ...
    def test_render_async_pz_vector_env(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
...
class TestAsyncPettingZooVecEnvGetAttr:
    @pytest.mark.parametrize(
        "env_fns",
        [
            [
    ...
    def test_get_attr_async_pz_vector_env(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
...
class TestAsyncPettingZooVecEnvStep:
    ...
    def test_step_async_pz_vector_env(self, use_single_action_space, env_fns):
        num_envs = 2
        try:
            env = AsyncPettingZooVecEnv(env_fns)
            env.reset()
...
class TestAsyncPettingZooVecEnvReset:
    @pytest.mark.parametrize("seed", [1, None])
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_reset_async_pz_vector_env(self, seed, env_fns):
        num_envs = 2
        env = AsyncPettingZooVecEnv(env_fns)
    ...
    def test_async_vector_subenv_error(self):
        env_list = [
    ...
    def test_async_vector_subenv_init_error(self, env_fns):
...
class TestAsyncPettingZooVecEnvResetAsync:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_reset_async_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
...
class TestAsyncPettingZooVecEnvResetWait:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
...
class TestAsyncPettingZooVecEnvStepAsync:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_step_async_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env._state = AsyncState.WAITING_RESET
        with pytest.raises(AlreadyPendingCallError):
...
class TestAsyncPettingZooVecEnvCallAsync:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_call_async_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
...
class TestAsyncPettingZooVecEnvCall:
    @pytest.mark.parametrize(
        "env_fns",
        [
            [
    ...
    def test_call_async_pz_vector_env(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        num_envs = 2
        env.reset()
...
class TestAsyncPettingZooVecEnvSetAttr:
    @pytest.mark.parametrize(
        "env_fns",
        [
            [
    ...
    def test_set_attr_make_values_list(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
    ...
    def test_set_attr_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env._state = AsyncState.WAITING_CALL
        with pytest.raises(AlreadyPendingCallError):
...
class TestAsyncPettingZooVecEnvCloseExtras:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_close_extras_warning(self, env_fns):
    ...
    def test_close_extras_terminate(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env.reset_async()
        env._state = AsyncState.WAITING_RESET
...
class TestAsyncPettingZooVecEnvPollPipeEnvs:
    ...
    def test_poll_pipe_envs(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
    ...
    def test_poll_pipe_envs_ready(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
...
class TestAsyncPettingZooVecEnvAssertIsRunning:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_assert_is_running(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env.closed = True
...
class TestAsyncPettingZooVecEnvStepWait:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_step_wait_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env._state = AsyncState.DEFAULT
        with pytest.raises(NoAsyncCallError):
            env.step_wait()
...
class TestAsyncPettingZooVecEnvCallWait:
    @pytest.mark.parametrize(
        "env_fns",
        [[speaker_listener_like_env for _ in range(2)]],
    )
    def test_call_wait_exception(self, env_fns):
        env = AsyncPettingZooVecEnv(env_fns)
        env._state = AsyncState.DEFAULT
        with pytest.raises(NoAsyncCallError):
            env.call_wait()
...
class TestAsyncPettingZooVecEnvAddInfo:
    def test_add_info_dictionaries(self):
        info_list = [
            {
                "agent_0": {
        ...
        assert np.all(
            vector_infos["agent_0"]["action_mask"]
    ...
    def test_add_info_unknown_objects(self):
        info_list = [
            {"agent_0": "string"},
...
class TestAsyncWorker:
    ...
    def test_worker_step_simple(self):
        num_envs = 1
        env_fns = [
        ...
        actions = {
            agent: vec_env.single_action_space(agent).sample()
    ...
    def test_worker_step_autoreset(self):
        num_envs = 1
    ...
    def test_worker_runtime_error(self):
        num_envs = 1
        env_fns = [speaker_listener_like_env for _ in range(num_envs)]
        vec_env = AsyncPettingZooVecEnv(env_fns)
        env_fn = env_fns[0]
...
class TestObservations:
    def test_observations_vector(self):
        num_envs = 1
        ...
        assert np.all(
            np.concatenate(vals, axis=1) == np.ones((1, 14), dtype=np.float32)
        )
    ...
    def test_observations_image(self):
        ...
        test_obs = {
            agent: np.ones((1, 7, 7, 3), dtype=np.uint8) * 255
            for agent in vec_env.agents
        }
...
class TestPettingZooVecEnvReset:
    def test_vec_env_reset_not_implemented(self):
        ...
        with pytest.raises(
            NotImplementedError, match="Subclasses must implement reset"
        ):
            vec_env.reset()


class TestPettingZooVecEnvStep:
    def test_vec_env_step_not_implemented(self):
        vec_env = PettingZooVecEnv(
            3,
            dummy_observation_spaces,
            dummy_action_spaces,
    ...
    def test_vec_env_step_skips_nan_actions(self):
...
class TestPettingZooVecEnvRender:
    def test_vec_env_render(self):
        vec_env = PettingZooVecEnv(
            3,
            dummy_observation_spaces,
            dummy_action_spaces,
...
class TestPettingZooVecEnvClose:
    def test_vec_env_closed(self):
        vec_env = PettingZooVecEnv(
            3,
            dummy_observation_spaces,
            dummy_action_spaces,
...
class TestPettingZooVecEnvCloseExtras:
    def test_vec_env_close_extras(self):
        vec_env = PettingZooVecEnv(
            3,
            dummy_observation_spaces,
            dummy_action_spaces,
...
class TestPettingZooVecEnvUnwrapped:
    def test_vec_env_unwrapped(self):
        vec_env = PettingZooVecEnv(
            3,
            dummy_observation_spaces,
            dummy_action_spaces,
...
class TestAsyncPettingZooVecEnvDel:
    def test_delete_async_pz_vec_env(self):
        env_fns = [
            lambda: speaker_listener_like_env(continuous_actions=False)
            for _ in range(2)
        ]
...
class TestGetPlaceholderValue:
    ...
    def test_get_placeholder_value(self, transition_name):
    ...
    def test_get_placeholder_value_observation_without_spaces(self):
        output = get_placeholder_value(
            agent="speaker_0",
    ...
    def test_get_placeholder_value_unknown_transition(self):
...
class TestAsyncPettingZooVecEnvReplayBuffer:
    @pytest.mark.parametrize("env_cls", [DictSpaceTestEnv, TupleSpaceTestEnv])
    def test_replay_buffer_with_various_spaces(self, env_cls):
