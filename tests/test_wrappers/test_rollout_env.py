...
class TestRolloutEnvReset:
    ...
    def test_overflow_at_reset_refreshes_prompt_token_len(self) -> None:
        class BudgetRowEnv:
            def __init__(self) -> None:
                self.prompts = ["ab", "x" * 80]
            ...
            def reset(
                ...
                del seed
                return self.prompts[row_index or 0], {}
        ...
        short_len = obs["input_ids"].shape[1]
        assert w._last_full_prompt_token_len == short_len

        obs, _ = w.reset(row_index=1)

        assert obs == {}
        assert w.done is True
        assert w.full_ids is not None
        overflow_len = int(w.full_ids.shape[1])
        assert overflow_len > short_len
        assert w._last_full_prompt_token_len == overflow_len
...
class TestBatchRolloutEnvReset:
    ...
    def test_lock_step_window_follows_geometry_inside_the_slot_pool(self) -> None:
        vec_env = RolloutCollector(
            env_factory=_SyncStubEnv,
            batch_size=2,
            group_size=4,
        )
        vec_env.update_rollout_geometry(rollout_batch_size=1, group_size=4)
        vec_env.reset(seed=0)
        token_ids, *_rest = vec_env.get_trajectories()
        assert len(vec_env.envs) == 8
        assert len(token_ids) == 4
        vec_env.update_rollout_geometry(rollout_batch_size=2, group_size=4)
        vec_env.reset(seed=0)
        token_ids, *_rest = vec_env.get_trajectories()
...
class TestBatchRolloutEnvPerEpisode:
    ...
    def test_geometry_update_rejects_non_positive_sizes(self) -> None:
        vec_env = self._collector()
        with pytest.raises(ValueError, match="batch_size must be > 0"):
            vec_env.update_rollout_geometry(rollout_batch_size=0, group_size=1)
        with pytest.raises(ValueError, match="group_size must be > 0"):
            vec_env.update_rollout_geometry(rollout_batch_size=1, group_size=-1)
        assert vec_env.batch_size == 2
        assert vec_env.group_size == 2

    def test_set_group_seed_raises_while_episodes_are_active(self) -> None:
        vec_env = self._collector()
        vec_env.reset_episode("ep0", logical_slot=0)
        with pytest.raises(RuntimeError, match="still active"):
            vec_env.set_group_seed(2)
    ...
    def test_stale_step_never_applied_to_a_reused_slot(self) -> None:
...
class TestMixSeed:
    ...
    def test_consecutive_windows_are_far_apart(self) -> None:
...
class TestRolloutEnvSystemPromptFromInfo:
    """An env can hand back a system prompt in its reset info.

    The constructor rejects the same pairing up front; this is the other door
    into it, where the value arrives at runtime from the env rather than config.
    """

    def _env(self, *, apply_chat_template: bool) -> RolloutHarness:
    ...
    def test_adopted_when_the_chat_template_renders_it(self) -> None:
        env = self._env(apply_chat_template=True)

        env._adopt_system_prompt({"system_prompt": "be terse"})

        assert env._system_prompt == "be terse"

    def test_rejected_without_a_chat_template_to_render_it(self) -> None:
        # Raw text is already fully rendered, so there is no system turn to
        # prepend to -- silently dropping it would lose the env's instruction.
        env = self._env(apply_chat_template=False)

        with pytest.raises(ValueError, match="prepend the system turn there"):
            env._adopt_system_prompt({"system_prompt": "be terse"})

    def test_a_prompt_already_set_is_not_replaced(self) -> None:
        env = self._env(apply_chat_template=False)
        env._system_prompt = "from config"

        # No raise: the guard is only reached while adopting a new value.
        env._adopt_system_prompt({"system_prompt": "from env"})

        assert env._system_prompt == "from config"

    def test_info_without_a_system_prompt_is_ignored(self) -> None:
        env = self._env(apply_chat_template=False)

        env._adopt_system_prompt({})

        assert env._system_prompt is None


class TestRolloutEnvPhaseGuards:
    def test_a_decode_that_returns_no_string_is_rejected(self) -> None:
        """A batch-shaped decode would make gen_text a list, corrupting the turn."""

        class _ListDecodingTokenizer(_ChrTokenizer):
            def decode(self, ids, skip_special_tokens: bool = True):
                del skip_special_tokens
                return [chr(int(x)) for x in ids]

        w = bare_rollout_env()
        w.tokenizer = _ListDecodingTokenizer()
        ...
        with pytest.raises(TypeError, match="returns str"):
            w._step_prepare(torch.tensor([[65, 66, 67]], dtype=torch.long))

    def test_step_apply_before_reset_is_rejected(self) -> None:
        w = bare_rollout_env()
