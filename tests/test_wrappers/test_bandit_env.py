...
class TestBanditEnvInit:
    # BanditEnv class creates an environment from a dataset
    def test_create_environment(self):
        features = pd.DataFrame(
            np.random.uniform(0, 1, size=(10, 1)), columns=["features"]
        )
        targets = pd.DataFrame(
            np.random.randint(0, 2, size=(10, 1)), columns=["targets"]
        )
...
class TestBanditEnvStep:
    ...
    def test_return_state_reward(self):
        features = pd.DataFrame(
            np.random.uniform(0, 1, size=(10, 1)), columns=["features"]
        )
        targets = pd.DataFrame(
