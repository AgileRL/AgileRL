...
from agilerl.llm_envs.qa_dataset import QADatasetEnv
...
def test_observation_text_supports_openenv_and_mcp_shapes(
    ...
    """``process_observation`` handles plain text, MCP blocks, prompt, and error."""
...
def test_openenv_wrapper_reset_copies_inner_system_prompt() -> None:
    env = _CountingEnv()
    env.system_prompt = "be terse"
    obs = OpenEnvWrapper(env).reset()
    assert obs.metadata["system_prompt"] == "be terse"
...
def test_resolve_env_url_is_scheme_case_insensitive() -> None:
    url, server = resolve_env("HTTP://example.invalid/openenv")
    assert url == "HTTP://example.invalid/openenv"
    assert server is None
...
class TestRolloutEnvFromSpec:
    ...
    def test_url_forwards_system_prompt_from_env_config(self) -> None:
        """A hosted URL has no env object; ``system_prompt`` still has to reach the harness."""
        env = RolloutHarness.from_spec(
            "http://localhost:0",
            {"system_prompt": "be terse"},
            MiniTokenizer(),
        )
        assert env._system_prompt == "be terse"
        env.close()
...
class TestTaskAssigner:
    ...
    def test_assign_spreads_seeds_deterministically(self) -> None:
        """Window seeds are reproducible but far apart in seed space.
...
def test_close_drops_the_session_so_the_next_connect_redials() -> None:
...
def test_connect_error_strips_url_userinfo() -> None:
    url = "http://user:secret@envs:8000"  # gitleaks:allow
    ...
    assert "http://envs:8000" in message


def test_redial_clears_cached_state_from_the_previous_host() -> None:
    """A URL-provider move must not keep the previous host's dataset_size / tools."""
...
def test_redial_drops_the_session_before_sleeping(
...
def test_session_state_application_error_propagates() -> None:
    """An application-level ``state`` error is not mapped to an empty dataset."""
    client = _session_client(state_error=RuntimeError("no state channel"))
    with pytest.raises(RuntimeError, match="no state channel"):
...
def test_state_server_error_propagates() -> None:
    """An unsupported ``state`` message propagates; reset still uses the session."""
    client = _session_client(
...
def test_server_rejects_shared_env_with_concurrent_sessions() -> None:
    """A shared env cannot back concurrent sessions; they would race on one inner."""
    with pytest.raises(ValueError, match="make_env"):
        OpenEnvServer(_CountingEnv(), max_concurrent_envs=2)
    OpenEnvServer(_CountingEnv(), max_concurrent_envs=1)
...
def test_observation_text_renders_all_shapes() -> None:
    """Bare strings, MCP content blocks, prompt, and errors all render."""
...
def test_source_of_names_the_single_source_a_manifest_declares() -> None:
    """One manifest section, one env source."""
...
@pytest.mark.parametrize(
    ...
    [
        ("http://envs:8000", True),
        ("https://envs:8000", True),
        ("HTTP://envs:8000", True),
        ("HTTPS://envs.example/path", True),
        ("http://user:pass@envs:8000", True),
        ("pkg:Env", False),
        ("ftp://envs:8000", False),
        ("http://", False),
        ("http:///nohost", False),
...
def test_is_url_parses_http_https_with_hostname(spec: str, expected: bool) -> None:
    assert is_url(spec) is expected


def test_redact_url_userinfo_strips_credentials() -> None:
    with_userinfo = "http://user:secret@envs:8000/ws"  # gitleaks:allow
    assert redact_url_userinfo(with_userinfo) == "http://envs:8000/ws"
    assert redact_url_userinfo("https://envs:8000") == "https://envs:8000"
...
def test_server_hosts_qa_dataset_environment_as_is() -> None:
...
def test_observation_field_wins_over_guessing() -> None:
    """A named ``observation_field`` is read before the specified shapes."""
    from agilerl.llm_envs.observation import process_observation
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"message": "m"},
        {"observation": "o"},
        {"board": "you are playing wordle", "reward": 0.0, "done": False},
        {"stdout": "hi\n", "stderr": "", "exit_code": 0},
        {"message": "tool result", "role": "tool"},
        {"board": "a", "hint": "b"},
    ],
)
def test_observation_does_not_guess_unnamed_string_fields(obs: dict[str, Any]) -> None:
    """Leftover unique strings are not specified shapes; the episode gets empty."""
    from agilerl.llm_envs.observation import process_observation
...
def test_action_field_reaches_an_in_process_env_action_class() -> None:
    """The model's text is written to the env's declared action class, under its own field."""
    env = _CodeEnv()
...
def test_is_url_rejects_a_non_string_spec() -> None:
    # A manifest can carry any YAML scalar here; source detection must answer
    # "not a URL" rather than raise inside urlparse.
    assert is_url(None) is False
    assert is_url(123) is False


def test_resolve_env_sets_the_system_prompt_on_the_built_env() -> None:
    """``system_prompt`` is applied after building, not passed to the factory.

    A library factory (``gem.make``) rejects it as a kwarg, so it is set on the
    env the factory returned.
    """
...
def test_server_rejects_a_make_env_that_builds_nothing() -> None:
    """A factory returning None would otherwise host a null env per session."""
    server = OpenEnvServer(make_env=lambda: None)

    with pytest.raises(RuntimeError, match="requires env or make_env"):
        server.start()
