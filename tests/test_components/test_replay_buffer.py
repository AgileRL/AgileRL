...
class TestReplayBufferLen:
    def test_get_length_of_memory(self):
...
class TestReplayBufferAdd:
    ...
    def test_add_experience_when_buffer_full(self):
    ...
    def test_add_experience_when_cursor_wraps(self):
    ...
    def test_add_with_1d_tensors_reshaped_to_batch_1(self):
    ...
    def test_add_with_nested_tensordict_1d_values_reshaped(self):
    ...
    def test_add_vectorized_experiences(self):
...
class TestReplayBufferSample:
    ...
    def test_sample_experiences_with_images(self):
    ...
    def test_sample_experiences_with_dict_obs(self):
    ...
    def test_sample_with_indices(self):
...
class TestReplayBufferSize:
    def test_replay_buffer_size_setter(self):
        buffer = ReplayBuffer(max_size=10)
        data = TensorDict(
            {
...
class TestReplayBufferStorage:
    def test_replay_buffer_storage_property_after_init(self):
        buffer = ReplayBuffer(max_size=10)
        data = TensorDict(
            {
                "state": torch.tensor([1]),
...
class TestMultiStepReplayBufferInit:
    def test_nstep_buffer_initialization(self):
...
class TestMultiStepReplayBufferAdd:
    def test_nstep_buffer_add(self):
    ...
    def test_nstep_buffer_add_with_images(self):
...
class TestMultiStepReplayBufferGetNStepInfo:
    def test_nstep_reward_calculation(self):
        ...
        assert torch.all(
            n_step_data["next_state"] == buffer.n_step_buffer[2]["next_state"]
        )
        assert torch.all(n_step_data["done"] == buffer.n_step_buffer[2]["done"])

    def test_nstep_early_termination(self):
    ...
    def test_multistep_buffer_get_n_step_info_termination_key(self):
    ...
    def test_multistep_buffer_get_n_step_info_terminated_key(self):
        buffer = MultiStepReplayBuffer(max_size=100, n_step=3, gamma=0.99)
        buffer.reward_key = "reward"
        buffer.ns_key = "next_obs"
        for i in range(3):
...
class TestMultiStepReplayBufferSample:
    def test_nstep_sampling_with_indices(self):
...
class TestMultiStepReplayBufferSampleFromIndices:
    def test_multistep_buffer_sample_from_indices(self):
        buffer = MultiStepReplayBuffer(max_size=100, n_step=3, gamma=0.99)
        buffer.reward_key = "reward"
        buffer.done_key = "done"
        buffer.ns_key = "next_state"
...
class TestPrioritizedReplayBufferInit:
...
class TestPrioritizedReplayBufferAdd:
    ...
    def test_prioritized_buffer_add_batch(self):
...
class TestPrioritizedReplayBufferSample:
    def test_prioritized_buffer_with_image_observations(self):
    ...
    def test_prioritized_buffer_with_dict_observations(self):
...
class TestPrioritizedReplayBufferUpdatePriorities:
    ...
    def test_prioritized_buffer_update_priorities_small_priority_clamped(self):
...
class TestPrioritizedReplayBufferCalculateWeights:
...
class TestPrioritizedReplayBufferUpdatePriority:
    def test_prioritized_replay_buffer_update_priority_assert(self):
        buffer = PrioritizedReplayBuffer(max_size=10, alpha=0.6)
        data = TensorDict(
            {
                "state": torch.tensor([1]),
...
class TestMultiAgentReplayBufferNormalizeDims:
...
class TestMultiAgentReplayBufferImages:
...
class TestMultiAgentReplayBufferHeterogeneous:
...
class TestMultiAgentReplayBufferAgentCounts:
...
class TestMultiAgentReplayBufferTransitionFlat:
...
class TestMultiAgentReplayBufferTransitionDictObs:
