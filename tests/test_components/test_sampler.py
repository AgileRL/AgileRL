...
class TestSamplerInit:
    ...
    def test_distributed_sampler_with_non_replay_buffer_dataset(self):
        class FakeDataset:
            def __init__(self):
    ...
    def test_distributed_sampler_warns_for_non_replaydataset(self):
        class FakeDataset:
    ...
    def test_distributed_sampler_warns_for_non_dataloader_after_replacement(
        self, monkeypatch
    ):
        ...
        with pytest.warns(
            UserWarning, match="Dataset is not a torch DataLoader object"
        ):
            _ = BrokenSampler(
                memory=None, dataset=replay_dataset, dataloader=dataloader
            )

    def test_sampler_warns_for_invalid_per_memory(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sampler_module, "PrioritizedReplayBuffer", TogglePrioritized
        )
        with pytest.warns(
            UserWarning,
    ...
    def test_sampler_warns_for_invalid_nstep_memory(self, monkeypatch):
        class ToggleMeta(type):
            calls = 0
...
class TestSamplerSampleStandard:
    # Call sample_standard() method with valid batch_size
    def test_sample_standard_with_valid_batch_size(self):
        memory_size = 100
        device = "cpu"
...
class TestSamplerSamplePer:
    # Call sample_per() method with valid batch_size
    def test_sample_per_with_valid_batch_size(self):
        memory_size = 100
        alpha = 0.6
        device = "cpu"
...
class TestSamplerSampleNStep:
    # Call sample_n_step() method with valid batch_size
    def test_sample_n_step_with_valid_batch_size(self):
        memory_size = 10000
        n_step = 3
        gamma = 0.95
        device = "cpu"
...
class TestSamplerReplaceDataloaderCollateFn:
    # Test that the Sampler correctly replaces the collate function in a regular DataLoader
    def test_replace_dataloader_collate_fn(self):
        memory_size = 100
        batch_size = 3
    ...
    def test_replace_dataloader_collate_fn_preserves_optional_params(self):
...
class TestSamplerCreateDataloader:
    def test_create_dataloader_sets_default_collate(self):
        dataset = ReplayDataset(ReplayBuffer(20, "cpu"), batch_size=2)
