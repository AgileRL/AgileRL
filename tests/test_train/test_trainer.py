...
class TestArenaTrainerResolveEnvSpec:
    def test_llm_environment_raises_without_name(self):
        manifest = MagicMock()
...
class TestMakeEnvBranches:
    ...
    def test_standard_env_calls_make_env(self):
        """Duck-typed env specs still expose make_env()."""
        mock_env = MagicMock()
        trainer = LocalTrainer.__new__(LocalTrainer)
...
class TestManifestNetworkAttachmentGuards:
    def test_a_none_only_net_config_annotation_keeps_the_spec(self):
        ...
        manifest = TrainingManifest.model_construct(
            algorithm=algorithm,
            network=object(),
        )
        assert manifest.algorithm_with_network() is algorithm
