...
def _offline_array_mapping() -> dict[str, np.ndarray]:
    return {
        "rewards": np.zeros(2),
...
def _run_train_offline(vec_env: MagicMock, dataset: object) -> None:
...
def test_train_offline_closes_hdf5_dataset_and_env_on_teardown(tmp_path):
    vec_env = _mock_vec_env()
    hdf5_path = tmp_path / "offline.hdf5"
    arrays = _offline_array_mapping()
    with h5py.File(hdf5_path, "w") as dataset_file:
        for key, value in arrays.items():
            dataset_file.create_dataset(key, data=value)

    dataset = h5py.File(hdf5_path, "r")
    _run_train_offline(vec_env, dataset)

    assert not dataset.id.valid
    vec_env.close.assert_called_once()


def test_train_offline_accepts_in_memory_mapping_and_closes_env():
    vec_env = _mock_vec_env()

    _run_train_offline(vec_env, _offline_array_mapping())
