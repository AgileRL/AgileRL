...
class TestEvolvableNetworkInit:
    def test_network_incorrect_initialization(self, vector_space):
        with pytest.raises(AttributeError):
            InvalidCustomNetwork(vector_space)

    def test_network_encoder_cls_must_be_evolvable_module(self, vector_space):
    ...
    def test_network_initialization(self, observation_space, encoder_type, request):
    ...
    def test_simba_encoder_build(self, vector_space):
        net = SimBaCustomNetwork(vector_space)
        assert isinstance(net.encoder, EvolvableSimBa)

    def test_encoder_alias_resolution(self, vector_space):
...
class TestEvolvableNetworkForwardHead:
    def test_network_forward_head_raises_without_head_net(self, vector_space):
...
class TestEvolvableNetworkInitializeHiddenState:
    def test_network_initialize_hidden_state_raises_when_not_recurrent(
        self, vector_space
    ):
    ...
    def test_recurrent_network_hidden_state_cache_and_extract_features(
        self, vector_space
    ):
...
class TestEvolvableNetworkMutationMethods:
    @pytest.mark.parametrize(
        "observation_space",
        ["dict_space", "discrete_space", "vector_space", "image_space"],
    )
    def test_network_mutation_methods(self, observation_space, dummy_rng, request):
        observation_space = request.getfixturevalue(observation_space)
...
class TestEvolvableNetworkForward:
    @pytest.mark.parametrize(
        "observation_space",
        ["dict_space", "discrete_space", "vector_space", "image_space"],
    )
    def test_network_forward(self, observation_space: spaces.Space, request):
        observation_space = request.getfixturevalue(observation_space)
...
class TestEvolvableNetworkClone:
    @pytest.mark.parametrize(
        "observation_space",
        ["dict_space", "discrete_space", "vector_space", "image_space"],
    )
    def test_network_clone(self, observation_space: spaces.Space, request):
        observation_space = request.getfixturevalue(observation_space)
...
class TestEvolvableNetworkHeadConfig:
    def test_head_config_raises_without_head_net(self, vector_space):
        class NoHeadNet(EvolvableNetwork):
            def __init__(self, obs_space):
                super().__init__(obs_space)
...
class TestEvolvableNetworkBuildNetworkHead:
    def test_activation_forward_head_and_base_not_implemented(self, vector_space):
...
class TestEvolvableNetworkInitWeightsGaussian:
    def test_init_weights_gaussian_and_change_activation(self, vector_space):
        net = CustomNetwork(vector_space)
...
class TestEvolvableNetworkRecreateEncoder:
    def test_custom_encoder_warning_and_recreate(self, vector_space):
        with pytest.warns(UserWarning, match="does not contain `num_outputs`"):
            net_warn = CustomEncoderNetwork(
                vector_space, encoder_cls=NoNumOutputsEncoder
            )
...
class TestEvolvableNetworkExtractFeatures:
    def test_extract_features_without_hidden_state(self, vector_space):
        net = CustomNetwork(vector_space)
