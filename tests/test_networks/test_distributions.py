...
class TestTorchDistributionSample:
    @pytest.mark.parametrize("batch_size", [1, 4])
    def test_torch_distribution_discrete_sample_log_prob_entropy(self, batch_size):
    ...
    @pytest.mark.parametrize("batch_size", [1, 4])
    def test_torch_distribution_box_sample_log_prob_entropy(self, batch_size):
    ...
    @pytest.mark.parametrize("batch_size", [1, 4])
    def test_torch_distribution_multi_discrete_sample_log_prob_entropy(
        self, batch_size
    ):
    ...
    @pytest.mark.parametrize("batch_size", [1, 4])
    def test_torch_distribution_multi_binary_sample_log_prob_entropy(self, batch_size):
    ...
    def test_torch_distribution_sampled_action_stored(self):
...
class TestEvolvableDistributionGetDistribution:
    def test_evolvable_distribution_get_distribution_unsupported_raises(self):
    ...
    def test_evolvable_distribution_get_distribution_for_all_supported_spaces(self):
...
class TestEvolvableDistributionApplyMask:
    def test_evolvable_distribution_apply_mask_unsupported_raises(self):
    ...
    def test_evolvable_distribution_apply_mask_multidiscrete_and_multibinary(self):
        md_space = spaces.MultiDiscrete([2, 3])
        md_net = _DummyModule(4, 5)
        ed_md = EvolvableDistribution(action_space=md_space, network=md_net)
...
class TestEvolvableDistributionForward:
    ...
    def test_evolvable_distribution_forward_sample_false_returns_none_action(self):
        action_space = spaces.Discrete(3)
        net = _DummyModule(4, 3)
        ed = EvolvableDistribution(action_space=action_space, network=net)
        latent = torch.randn(2, 4)
    ...
    def test_evolvable_distribution_forward_mask_stack_failure_raises(self):
        action_space = spaces.Discrete(3)
        net = _DummyModule(4, 3)
        ed = EvolvableDistribution(action_space=action_space, network=net)
        latent = torch.randn(2, 4)
...
class TestEvolvableDistributionLogProb:
    def test_evolvable_distribution_log_prob_entropy_require_forward(self):
        action_space = spaces.Discrete(3)
        net = _DummyModule(4, 3)
        ed = EvolvableDistribution(action_space=action_space, network=net)
    ...
    def test_evolvable_distribution_log_prob_entropy_after_forward(self):
        action_space = spaces.Discrete(3)
        net = _DummyModule(4, 3)
        ed = EvolvableDistribution(action_space=action_space, network=net)
        latent = torch.randn(2, 4)
...
class TestEvolvableDistributionClone:
    def test_evolvable_distribution_clone(self):
        action_space = spaces.Discrete(3)
        net = _DummyModule(4, 3)
        ed = EvolvableDistribution(action_space=action_space, network=net)
        clone = ed.clone()
