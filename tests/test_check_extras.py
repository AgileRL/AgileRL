...
"""The agilerl-arena requirement must be a compatible range covering the version this release cuts."""
...
HUB_SEMVER_SH = Path(__file__).resolve().parents[3] / "scripts" / "lib" / "semver.sh"
...
def hub_semver(check_extras: ModuleType, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    if not HUB_SEMVER_SH.is_file():
        pytest.skip(f"hub semver helper not in this checkout: {HUB_SEMVER_SH}")
    monkeypatch.setattr(check_extras, "_hub_semver_script", lambda: HUB_SEMVER_SH)
    return HUB_SEMVER_SH
...
def _write_pyproject(
    ...
    *,
    ...
    deps_toml = f"dependencies = [{', '.join(arena)}]\n"
    if extras is None:
...
def test_arena_range_outside_tag_fails(
    ...
    _assert_exit_err(
        capsys,
        "outside the requirement range",
        check_extras._check_arena_requirement,
        reqs,
        require_tags=True,
    )
...
def test_git_tag_failure_without_stderr_fails_closed(
    ...
    reqs = check_extras._arena_requirements(pyproject)
    _assert_exit_err(
        capsys, "exit 1", check_extras._check_arena_requirement, reqs, require_tags=True
    )


def test_missing_arena_requirement_fails(
    check_extras: ModuleType, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """agilerl-arena is a base dependency; dropping it must fail, not pass quietly."""
    pyproject = _write_pyproject(
        tmp_path,
        arena_reqs=[],
        extras={"box2d": ["box2d-py"], "all": ["agilerl[box2d]"]},
    )
    _bind_repo(check_extras, tmp_path, pyproject)
    reqs = check_extras._arena_requirements(pyproject)
    _assert_exit_err(capsys, "Missing", check_extras._check_arena_requirement, reqs)
...
def test_committed_exact_pin_fails(
    ...
    reqs = check_extras._arena_requirements(pyproject)
    _assert_exit_err(
        capsys, "compatible range", check_extras._check_arena_requirement, reqs
    )
...
def test_missing_range_fails(
    ...
    reqs = check_extras._arena_requirements(pyproject)
    _assert_exit_err(
        capsys, "Expected one", check_extras._check_arena_requirement, reqs
    )
...
def test_arena_pin_override_matches(
    ...
    reqs = check_extras._arena_requirements(pyproject)
    assert check_extras._check_arena_requirement(reqs) == "==0.2.0+build.9"
...
def test_arena_pin_override_requires_exact(
    ...
    reqs = check_extras._arena_requirements(pyproject)
    _assert_exit_err(
        capsys, "AGILERL_ARENA_PIN", check_extras._check_arena_requirement, reqs
    )
...
def test_portable_skips_missing_tags(check_extras: ModuleType, tmp_path: Path) -> None:
    ...
    _bind_repo(check_extras, tmp_path, pyproject)
    reqs = check_extras._arena_requirements(pyproject)
    assert check_extras._check_arena_requirement(reqs) == ">=0.2.0,<0.3"


# Arena version this release cuts.
