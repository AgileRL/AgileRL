...
from agilerl.networks.evolvable_mlp import NoisyLinear
...
def test_create_mlp(output_vanish, noisy, device):
    evolvable_mlp = EvolvableMLP(num_inputs=10, 
                                 ...
                                 support=torch.linspace(0.0, 200.0, 50).to(device),
                                 device=device)
    net = evolvable_mlp.create_mlp(10, 4, [32,32], output_vanish=output_vanish, output_activation=None, noisy=noisy)
...
def test_add_nodes(num_inputs, num_outputs, hidden_size, hidden_layer, numb_new_nodes, device):
...
def test_remove_nodes(num_inputs, num_outputs, hidden_size, hidden_layer, numb_new_nodes, device):
    mlp = EvolvableMLP(num_inputs=num_inputs, 
                       num_outputs=num_outputs, 
                       hidden_size=hidden_size,
                       min_mlp_nodes=2,
                       device=device)
    original_hidden_size = copy.deepcopy(mlp.hidden_size)
    result = mlp.remove_mlp_node(numb_new_nodes=numb_new_nodes, hidden_layer=hidden_layer)
    hidden_layer = result["hidden_layer"]
