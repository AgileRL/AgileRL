...
class TestMutationsInit:
    ...
    def test_grouped_dataclasses_match_flat_kwargs(self):
        ...
        assert grouped.no_mut == flat.no_mut == positional_leftover.no_mut == 0.1
        assert (
            grouped.architecture_mut
            == flat.architecture_mut
            == positional_leftover.architecture_mut
            == 0.2
        )
        assert (
            grouped.new_layer_prob
            == flat.new_layer_prob
            == positional_leftover.new_layer_prob
            == 0.3
        )
        assert (
            grouped.parameters_mut
            == flat.parameters_mut
            == positional_leftover.parameters_mut
            == 0.4
        )
        assert (
            grouped.activation_mut
            == flat.activation_mut
            == positional_leftover.activation_mut
            == 0.5
        )
        assert (
            grouped.rl_hp_mut == flat.rl_hp_mut == positional_leftover.rl_hp_mut == 0.6
        )
        assert (
            grouped.mutation_sd
            == flat.mutation_sd
            == positional_leftover.mutation_sd
            == 0.7
        )
        assert (
            grouped.activation_selection
            == flat.activation_selection
            == positional_leftover.activation_selection
        )
        assert grouped.device == flat.device == positional_leftover.device == "cpu"
