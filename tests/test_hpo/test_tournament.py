...
class TestTournamentSelectionSelect:
    ...
    def test_language_model_tournament(self):
        ...
        population = [
            GRPO(
                ...
                offload_trainer_during_rollout=init_hp.get(
                    "OFFLOAD_TRAINER_DURING_ROLLOUT",
                    False,
                ),
...
class TestTournamentAgentEviction:
    def test_cleanup_skips_empty_slots(self):
        from agilerl.hpo.tournament import _cleanup_unwanted_llm_slots

        _cleanup_unwanted_llm_slots([None], [0], {0})
