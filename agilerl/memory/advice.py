...
knob mutation that is re-run through the estimator; the reported saving is
the exact delta the model predicts. This keeps advice automatically
consistent with the calculation core and restricted to knobs that actually
...
def advise(
    ...
    """Rank knob changes by the memory they save on the given phase (or on
