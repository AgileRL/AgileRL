...
class LLMEnvSpec(BaseModel):
    ...
    * ``env_url`` -- a URL to an already-hosted OpenEnv service (driven over
    ...
    service (a container, a Space, or a server a Ray actor stands up) and point
    ``env_url`` at it. Transport is thus a deployment concern, not a code
    change: the same env trains in-process for dev and against a hosted URL in
    production.
    ...
    :param env_packages: What the entrypoint needs installed, as ``{"uv": [...]}``
        ...
        the first time the env turns out not to be importable. An orchestrator
        reads the same field as a dedicated environment to build for the env
        instead, so a manifest naming it runs either way.
    ...
    def _validate_rollout_fields(self) -> Self:
        ...
        if self.env_packages is not None:
            from agilerl.llm_envs.env_packages import package_list
    ...
    def make_rollout_env_factory(
        ...
        *,
        ...
        dataset rows plus a reward function, or a callable resolved from
        ``entrypoint`` and built with ``env_config``.
