    ...
    def _collect_fitnesses(self) -> ScalarOrNestedRow:
        ...
        for agent in self.agents:
            ...
            if isinstance(latest_fitness, dict):  # multi-agent -> sum_scores=False
                ...
                sub_fitness = list(latest_fitness)
                fitnesses.append(
                    {
                        agent_id: float(sub_fitness[idx])
        ...
        # Every agent takes the same branch above, so the row is homogeneous.
        return cast("ScalarOrNestedRow", fitnesses)
