...
"""Record types for collected LLM rollouts, collated one prompt group at a time.
...
each group is a single trajectory). Groups are collated whole, so the
...
padding into rectangular batches happens once, in :func:`collate_rollout_groups`.
"""
