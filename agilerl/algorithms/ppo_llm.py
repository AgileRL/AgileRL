...
class PPO(LLMAlgorithm):
    ...
    :param importance_sampling_level: IS / ratio-pooling level for the policy
        ...
        (``action_granularity``). ``"token"`` clips per token; ``"turn"`` pools
    ...
    def __init__(
        ...
        # GAE advantage granularity (``action_granularity``/``ppo_granularity``).
    ...
    def get_action(
        ...
        **kwargs: Any,
        ...
        :return: An :class:`ActionResult` of per-prompt completion token IDs and
            masks (PPO does not use the sampling-mismatch correction, so its
            ``sampling_logps`` is always ``None``).
        :rtype: ActionResult
        """
