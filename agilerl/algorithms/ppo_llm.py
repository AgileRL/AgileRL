    ...
    def learn(
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, num_samples, batch_size):
                ...
                # print("Propotion of desired token in batch", ((batch_ids == target_token_id).sum(dim=-1).float() / batch_action_mask.sum(dim=-1)).mean())

                # reporting_reward += (((batch_ids[:, 1:] * batch_action_mask) == target_token_id).sum(dim=-1).float() / batch_action_mask.sum(dim=-1)).mean()

                # terminal_idx = batch_action_mask.long().cumsum(dim=1).max(dim=1)[1]
                # terminal_values = batch_values.gather(1, terminal_idx.unsqueeze(1)).squeeze(1)

                # print("Terminal reward", batch_token_rewards.gather(1, terminal_idx.unsqueeze(1)).squeeze(1))
                # print("Terminal reward -1", batch_token_rewards.gather(1, terminal_idx.unsqueeze(1) - 1).squeeze(1))
                # print("Terminal value", terminal_values)
                # Set torch print options so we can see the full tensor
                # torch.set_printoptions(threshold=torch.inf)
                # print("Batch returns", batch_returns)
                # print("Batch returns multi by action mask", batch_returns * batch_action_mask)
                # assert False

                # print("Batch advantages", batch_advantages[0])
                # print("Batched returns", batch_returns[0])
                # print("Batch values", batch_values[0])
                # assert False
