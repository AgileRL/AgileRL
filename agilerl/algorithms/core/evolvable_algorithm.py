...
class EvolvableAlgorithm(
    EvolvableCheckpointMixin, ABC, Generic[ExperiencesT], metaclass=RegistryMeta
):
    ...
    def _reinit_opt_from_config(
        ...
        if hasattr(self, "cosine_lr_schedule_config"):
            lr = (
                tuple(getattr(self, lr_name) for lr_name in config.lr)
                if isinstance(config.lr, tuple)
                else getattr(self, config.lr)
            )
            update_lr_fn = getattr(type(self), "update_lr", update_optimizer_lr)
            self.accelerator, self.lr_scheduler = update_lr_fn(
                optimizer,
                lr=lr,
                accelerator=self.accelerator,
