...
(``LigerFusedLinearPolicyLossFunction``, ``llm_policy_loss_fn``)
requires ``liger-kernel`` at import time, so it is gated on
:data:`agilerl.HAS_LIGER_KERNEL`. Without Liger the public symbols
resolve to ``None`` so callers' ``is None`` guard fires.
"""
