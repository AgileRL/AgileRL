...
if TYPE_CHECKING or HAS_LLM_DEPENDENCIES:
    ...
    from agilerl.algorithms.core.llm_ops.fused_lora import (
        ...
        interleaved_sequence_adapter_plan,
...
class LLMAlgorithm(EvolvableAlgorithm[ExperiencesT], ABC, Generic[ExperiencesT]):
    ...
    def __init__(
        ...
        if fsdp_config is not None:
            # FSDP and the later vLLM PyNccl weight-sync group must use the
            # same cuMem setting or ncclCommInitRank fails. Disable gIB plugins
            # before the first NCCL init; they abort InitRank on Ethernet
            # without NCCL_TUNER_CONFIG_PATH.
            os.environ.setdefault("NCCL_CUMEM_ENABLE", "0")
            os.environ.setdefault("NCCL_TUNER_PLUGIN", "none")
            os.environ.setdefault("NCCL_NET_PLUGIN", "none")
            os.environ.setdefault("NCCL_PROFILER_PLUGIN", "none")
    ...
    def _export_optimizer_state(self, cpu_offload: bool = True) -> dict[str, Any]:
        ...
        Under FSDP2, ``cpu_offload`` must be ``True``: only rank 0 holds the
        consolidated optimizer state on CPU (no full GPU gather on every rank).
    ...
    def _copy_clone_attributes(self, clone: Self) -> Self:
        ...
        :rtype: Self
        """
        cloned_actor = clone.actor
        cloned_shard_runtime = clone.shard_runtime
        cloned_optimizer = clone.optimizer
        cloned_lr_scheduler = clone.lr_scheduler
        original_actor = self.actor
        original_shard_runtime = self.shard_runtime
        original_optimizer = self.optimizer
        original_lr_scheduler = self.lr_scheduler

        clone.actor = None
        clone.shard_runtime = None
        ...
        clone = EvolvableAlgorithm.copy_attributes(self, clone)
        clone.actor = cloned_actor
        clone.shard_runtime = cloned_shard_runtime
        clone.optimizer = cloned_optimizer
        clone.lr_scheduler = cloned_lr_scheduler
        self.actor = original_actor
        self.shard_runtime = original_shard_runtime
    ...
    def _assemble_fused_chunks(
        ...
        restore_rows: list[int] | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor | None]:
        """Write micro-batch logprobs into one preallocated buffer.

        :param restore_rows: Source row indices of a mix permute; invert so
            outputs match the adapter-major input order.
        """
        ...
        if restore_rows is not None:
            # Put rows back in original order: all refs, then all actors.
            src = torch.tensor(restore_rows, device=logprobs_out.device)
            inv = torch.empty(total, dtype=torch.long, device=logprobs_out.device)
            inv[src] = torch.arange(total, device=logprobs_out.device)
            logprobs_out = logprobs_out.index_select(0, inv)
            if values_out is not None:
                values_out = values_out.index_select(0, inv)
    ...
    def _fused_model_pass(
        ...
        ``no_grad``. Dense fused LoRA interleaves adapter runs so each chunk
        is one sequence x all adapters; packed-expert LoRA stays
        adapter-aligned.
        ...
        if batch_size is None:
            ...
            any(
                _is_packed_experts_module(module)
                for module in torch.nn.Module.modules(self.actor)
            )
            or (plan := interleaved_sequence_adapter_plan(routing)) is None
        ):
            chunks = adapter_aligned_chunks(routing, batch_size)
        else:
            routing, chunks, restore_rows = plan
            index = torch.tensor(restore_rows, device=fused_ids.device)
            fused_ids = fused_ids.index_select(0, index)
            fused_mask = fused_mask.index_select(0, index)
        ...
        with self.shard_runtime.gather_layer(
            ...
            if len(chunks) == 1 and restore_rows is None:
                return _run_chunk(0, total)
            return self._assemble_fused_chunks(
                chunks, total, seq_len_out, _run_chunk, restore_rows
            )
