...
class REINFORCE(LLMAlgorithm):
    ...
    def learn(
        ...
        :return: Dict with keys ``loss``, ``kl``, ``entropy`` (plus ``reward`` and
            ``completion_length``), averaged over all minibatch updates.
        ...
        self.metrics.log("kl", agg["kl"])
        self.metrics.log("entropy", agg["entropy"])
        self.metrics.log("reward", agg["reward"])
        self.metrics.log("completion_length", agg["completion_length"])
