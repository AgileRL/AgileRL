...
class _ReplayBatch(TypedDict, total=False):
    """Field layout of an off-policy replay sample (components/replay_buffer.py).

    ``weights`` and ``idxs`` are only present for prioritized (or
    ``return_idx=True``) sampling.
    """
...
class RainbowDQN(RLAlgorithm):
    ...
    def __init__(
        ...
        if actor_network is not None:
            ...
            def create_actor() -> RainbowQNetwork:
                # NetworkMeta.__call__ (agilerl/networks/base.py) types constructor
                # calls as EvolvableNetwork; fix upstream to return the subclass.
    ...
    def learn(
        ...
        n_batch = cast("_ReplayBatch | None", n_experiences)
