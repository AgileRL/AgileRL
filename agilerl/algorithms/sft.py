...
class SFT(LLMAlgorithm):
    ...
    def learn(
        ...
        if self.accelerator is not None:
            ...
            agg_perplexity = safe_aggregate_metrics(self.accelerator, mean_perplexity)
        else:
            agg_loss = mean_loss
            agg_perplexity = mean_perplexity

        if training:
            self.metrics.log("loss", agg_loss)
            self.metrics.log("perplexity", agg_perplexity)

        return agg_loss, agg_perplexity
