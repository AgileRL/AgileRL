...
class PPO(RLAlgorithm):
    ...
    def __init__(
        ...
        if actor_network is not None and critic_network is not None:
            ...
            actor_copy, critic_copy = make_safe_deepcopies(
                ...
                critic_network,  # ty: ignore[invalid-argument-type]
            )
    ...
    def share_encoder_parameters(self) -> None:
        ...
        if isinstance(self.actor, EvolvableNetwork) and isinstance(
            ...
            # EvolvableNetwork satisfies the protocol at runtime; the match
            # fails only on the mutable `device` member (`str` vs
            # `DeviceType`) — upstream fix belongs in agilerl/protocols.py.
            share_encoder_parameters(self.actor, self.critic)  # ty: ignore[invalid-argument-type]  # concrete networks can't satisfy EvolvableNetworkProtocol: its clone() is declared to return the protocol, not Self — fix in agilerl/protocols.py
        else:
            warnings.warn(
        ...
        *,
        ...
        if hidden_state is not None:
            if self.share_encoders:
                # When sharing encoders, both networks use the same hidden
                # state. Recurrent encoders return (latent, hidden-state
                # dict); the tuple[Tensor, Tensor] overload of
                # extract_features under-types the second element (upstream
                # fix in agilerl/networks/base.py).
                latent_pi, next_hidden_actor = cast(
                    "tuple[torch.Tensor, dict[str, torch.Tensor]]",
                    self.actor.extract_features(obs, hidden_state=hidden_state),
                )
                ...
                # Forward pass through actor with its hidden state (same
                # extract_features hidden-dict under-typing as above).
                latent_pi, next_hidden_actor = cast(
                    "tuple[torch.Tensor, dict[str, torch.Tensor]]",
                    self.actor.extract_features(obs, hidden_state=actor_hidden_state),
                )
                action, log_prob, entropy = self.actor.forward_head(
                    latent_pi,
                    action_mask=action_mask,
                ...
                # Forward pass through critic with its hidden state; the
                # recurrent forward returns (values, hidden-state dict) while
                # EvolvableNetwork.__call__ is annotated Tensor-only
                # (upstream fix in agilerl/networks/base.py).
                values, next_hidden_critic = cast(
                    "tuple[torch.Tensor, dict[str, torch.Tensor]]",
                    self.critic(obs, hidden_state=critic_hidden_state),
                )
    ...
    # return, which cannot express PPO's (action, log_prob, entropy, values,
    # ...) tuples; the upstream fix is to return ActionReturnType in
    ...
    def _learn_from_rollout_buffer_flat(
        ...
        if buffer_td_external is not None:
            ...
            # get_tensor_batch returns a TensorDict at runtime; its
            # dict-of-tensors annotation is the pending upstream fix
            # (agilerl/components/rollout_buffer.py).
            buffer_td = cast(
                "TensorDict",
                self.rollout_buffer.get_tensor_batch(device=str(self.device)),
            )
    ...
    # return, but every algorithm returns a scalar fitness; the upstream fix
    # is to widen the base annotation in agilerl/algorithms/core/base.py.
    def test(
        self,
