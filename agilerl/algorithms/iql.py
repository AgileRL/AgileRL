...
class DiagGaussianActor(nn.Module):
    """Actor with a Gaussian policy.
    
    :param network: Pre-Gaussian evolvable network of actor
    :type network: nn.Module()
    :param output_dim: Output dimension of actor - typically the action dim
    :type output_dim: int
    :param log_std_min: Minimum value for clamping log std
    :type log_std_min: float
    :param log_std_max: Maximum value for clamping log std
    :type log_std_max: float
    """
    def __init__(self, network, output_dim, log_std_min=-10, log_std_max=2):
        super(DiagGaussianActor, self).__init__()
        self.network = network
        self.output_dim = output_dim
        self.mu = nn.Linear(network.hidden_size[-1], output_dim)
        self.log_std_linear = nn.Linear(network.hidden_size[-1], output_dim)
...
class IQL():
    """The IQL algorithm class. IQL paper: https://arxiv.org/abs/2110.06169
    ...
    def __init__(self, state_dim, action_dim, one_hot, max_action, expl_noise=0.1, index=0, 
        ...
        if self.net_config['arch'] == 'mlp':      # Multi-layer Perceptron
            actor_pre_net = EvolvableMLP(
                num_inputs=state_dim[0],
                num_outputs=self.net_config['h_size'][-1],
                hidden_size=self.net_config['h_size'],
                output_activation='tanh',
                device=self.device,
                accelerator=self.accelerator)
            self.actor = DiagGaussianActor(network=actor_pre_net,
            ...
            actor_pre_net = EvolvableCNN(
                input_shape=state_dim,
                num_actions=self.net_config['h_size'][-1],
        ...
        self.value_net_optimizer_type = optim.Adam(self.value_net.parameters(), lr=self.lr)
        ...
        if self.accelerator is not None:
            ...
            self.value_net_optimizer = self.value_net_optimizer_type
            if wrap:
                self.wrap_models()          
        else:
    ...
    def clone(self, index=None, wrap=True):
        ...
        value_net = self.value_net.clone()
        ...
        value_net_optimizer = optim.Adam(clone.value_net.parameters(), lr=clone.lr)
        ...
        clone.value_net_optimizer_type = value_net_optimizer

        if self.accelerator is not None:
            if wrap:
                clone.actor, clone.critic_1, clone.critic_target_1, \
                clone.critic_2, clone.critic_target_2, clone.value_net, clone.actor_optimizer, \
                clone.critic_1_optimizer, clone.critic_2_optimizer, clone.value_net_optimizer = self.accelerator.prepare(
                                                                                            actor,
                                                                                            critic_1,
                                                                                            critic_target_1,
                                                                                            critic_2,
                ...
                clone.actor, clone.critic_1, clone.critic_target_1, clone.critic_2, \
                clone.critic_target_2, clone.value_net, clone.actor_optimizer, clone.critic_1_optimizer, \
                clone.critic_1_optimizer, clone.value_net_optimizer = actor, critic_1, \
                critic_target_1, critic_2, critic_target_2, value_net, actor_optimizer, critic_1_optimizer, \
                critic_2_optimizer, value_net_optimizer
        else:
            ...
            clone.value_net = value_net.to(self.device)
            ...
            clone.value_net_optimizer = value_net_optimizer
    ...
    def wrap_models(self):
        if self.accelerator is not None:
            self.actor, self.critic_1, self.critic_target_1, self.critic_2, \
            self.critic_target_2, self.value_net, self.actor_optimizer, \
            self.critic_1_optimizer, self.critic_2_optimizer, self.value_net_optimizer = self.accelerator.prepare(
                                                                                        self.actor,
                                                                                        self.critic_1,
    ...
    def unwrap_models(self):
        if self.accelerator is not None:
            ...
            self.value_net = self.accelerator.unwrap_model(self.value_net)
            ...
            self.value_net_optimizer = self.accelerator.unwrap_model(self.value_net_optimizer)
    ...
    def saveCheckpoint(self, path):
        ...
        """
        torch.save({
            'actor_pre_net_init_dict': self.actor.network.init_dict,
            ...
            'value_net_init_dict': self.value_net.init_dict,
    ...
    def loadCheckpoint(self, path):
        ...
        if self.net_config['arch'] == 'mlp':
            actor_pre_net = EvolvableMLP(**checkpoint['actor_pre_net_init_dict'])
            self.actor = DiagGaussianActor(actor_pre_net, **checkpoint['actor_init_dict'])
            ...
            self.value_net = EvolvableMLP(**checkpoint['value_net_init_dict'])
        elif self.net_config['arch'] == 'cnn':
            actor_pre_net = EvolvableCNN(**checkpoint['actor_init_dict'])
            self.actor = DiagGaussianActor(actor_pre_net, **checkpoint['actor_init_dict'])
            ...
            self.value_net = EvolvableCNN(**checkpoint['value_net_init_dict'])
