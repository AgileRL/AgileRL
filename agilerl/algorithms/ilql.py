...
class ILQL(nn.Module):
    ...
    def __init__(self, dataset, 
                 net_config={'arch':'gpt', 
                             ...
                             'max_layers': 16,
                             'bias': True,
                             }, index=0, batch_size=64, lr=1e-4, learn_step=5, alpha=0.005, beta=0., gamma=0.99, tau=0.6, 
                             mutation=None, transition_weight=0., clip_weight=None, value_max=None, value_min=None, 
                             detach_v=False, detach_q=False, detach_pi=False, double_q=True, per_token=True, exp_weights=True, 
                             dm_margin=0., cql_temp=1., weight_decay=0., device='cpu'):
        ...
        end2end = False if per_token else True

        # model
        self.model = EvolvableGPT(n_layer=net_config['n_layer'], vocab_size=net_config['vocab_size'], n_embd=net_config['n_embd'], n_head=net_config['n_head'], 
                                  dim_feedfwd=net_config['dim_feedfwd'], block_size=net_config['block_size'], dropout=net_config['dropout'], 
                                  activation=net_config['activation'], layer_norm_eps=net_config['layer_norm_eps'], min_layers=net_config['min_layers'], 
                                  max_layers=net_config['max_layers'], bias=net_config['bias'], device=self.device).to(self.device)
        # lm policy
        self.actor = EvolvableGPT(n_layer=net_config['n_layer'], vocab_size=net_config['vocab_size'], n_embd=net_config['n_embd'], n_head=net_config['n_head'], 
                                  dim_feedfwd=net_config['dim_feedfwd'], block_size=net_config['block_size'], dropout=net_config['dropout'], 
                                  activation=net_config['activation'], layer_norm_eps=net_config['layer_norm_eps'], min_layers=net_config['min_layers'], 
                                  max_layers=net_config['max_layers'], bias=net_config['bias'], device=self.device).to(self.device)
        # lm target
        self.actor_target = EvolvableGPT(n_layer=net_config['n_layer'], vocab_size=net_config['vocab_size'], n_embd=net_config['n_embd'], n_head=net_config['n_head'], 
                                         dim_feedfwd=net_config['dim_feedfwd'], block_size=net_config['block_size'], dropout=net_config['dropout'], 
                                         activation=net_config['activation'], layer_norm_eps=net_config['layer_norm_eps'], min_layers=net_config['min_layers'], 
                                         max_layers=net_config['max_layers'], bias=net_config['bias'], device=self.device).to(self.device)
        ...
        # v and q networks
        self.v = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                              device=self.device).to(self.device)
        self.q = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                              device=self.device).to(self.device)
        self.target_q = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                                     device=self.device).to(self.device)
        self.target_q.load_state_dict(self.q.state_dict())

        if self.double_q:
            self.q2 = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                                   device=self.device).to(self.device)
            self.target_q2 = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                                          device=self.device).to(self.device)
            self.target_q2.load_state_dict(self.q2.state_dict())

        self.pi = EvolvableMLP(num_inputs=net_config['d_model'], num_outputs=1, hidden_size=[net_config['d_model']*2, net_config['d_model']*2], 
                               device=self.device).to(self.device)
       
        self.optimizer = optim.AdamW(self.actor.parameters(), lr=self.lr, weight_decay=self.weight_decay)

    def forward(self, tokens: torch.Tensor, state_idxs: torch.Tensor, action_idxs: torch.Tensor, attn_mask: Optional[torch.Tensor]=None, 
                prefix_embs: Optional[torch.Tensor]=None, skip_policy_on_train: bool=False,  detach_full_policy: bool=False):
        ...
        model_outputs, model_hidden_states = self.model.transformer(tokens, attn_mask)        
        hidden_states = model_hidden_states[-1]
        
        with torch.no_grad():
            target_outputs, target_hidden_states = self.actor_target.transformer(tokens, attn_mask) 
        ...
        if skip_policy_on_train and self.training:
            ...
            if detach_full_policy:
                with torch.no_grad():
                    policy_outputs, policy_hidden_states = self.actor.transformer(tokens, attn_mask)
            else:
                policy_outputs, policy_hidden_states = self.actor.transformer(tokens, attn_mask)
        ...
        state_hidden_states = torch.gather(input=hidden_states, dim=1, index=state_idxs.unsqueeze(2).repeat(1, 1, self.net_config['d_model']))
        action_hidden_states = torch.gather(input=hidden_states, dim=1, index=action_idxs.unsqueeze(2).repeat(1, 1, self.net_config['d_model']))
        action_target_hidden_states = torch.gather(input=target_hidden_states, dim=1, index=action_idxs.unsqueeze(2).repeat(1, 1, self.net_config['d_model']))
    ...
    def softUpdate(self):
        ...
        if self.double_q:
            for target_param, local_param in zip(self.target_q2.parameters(), self.q2.parameters()):
                target_param.data.copy_(self.alpha*local_param.data + (1.0-self.alpha)*target_param.data)
        if self.lm_target is not None:
            for target_param, local_param in zip(self.actor_target.parameters(), self.model.parameters()):
                target_param.data.copy_(self.alpha*local_param.data + (1.0-self.alpha)*target_param.data)
    ...
    def clone(self, index=None):
        ...
        clone.optimizer = optim.Adam(clone.actor.parameters(), lr=clone.lr, weight_decay=clone.weight_decay)
