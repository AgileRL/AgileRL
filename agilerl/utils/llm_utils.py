...
from agilerl.distributed.process import (
    parameter_owner as _parameter_owner,
)
...
StateDictOptions: Any
get_model_state_dict: Any
try:
...
def safe_aggregate_metrics(
    ...
    """Aggregate a metric across ranks, or locally on a single device."""
    return aggregate_metrics_across_gpus(metrics)
...
def is_fsdp_sharded(model: nn.Module) -> bool:
    ...
    Detects both FSDP2 (``fully_shard``) and FSDP1 wrappers; AgileRL
...
def _is_dtensor(tensor: torch.Tensor) -> TypeGuard[DTensor]:
...
def save_lora_adapters(
    ...
    """
    # peft / safetensors: optional extras this adapter-export path needs.
    ...
    for adapter_name in selected_adapters:
        ...
        if is_main:
            ...
            if (
                ...
                and adapter_name in peft_model.peft_config
            ):
                peft_model.peft_config[adapter_name].save_pretrained(str(adapter_dir))
...
def load_lora_adapters(
    ...
    """
    # safetensors is an optional extra this adapter-load path needs.
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            ...
            if hasattr(param, "device_mesh"):
                ...
                getattr(param.data, local_tensor).copy_(getattr(sharded, local_tensor))
            else:
...
def install_packed_expert_grouped_gemm(model: nn.Module) -> int:
    """Bind grouped GEMM on packed-expert modules. No-op on dense models.

    Imported lazily: ``moe_lora`` lives under ``agilerl.algorithms``, and
    loading that package imports ``base``, which imports ``llm_envs``.
    """
    from agilerl.algorithms.core.llm_ops.moe_lora import (
        install_packed_expert_grouped_gemm as install,
    )
...
def create_model_from_name_or_path(
    ...
    install_packed_expert_grouped_gemm(model)
    return model
...
def build_completion_mask(
    ...
    :param prompt_len: Number of leading tokens to mask out (the full
        ...
        completion. ``prompt_len is None`` treats every non-pad token as
        completion (the full sequence is marked, then pads are cleared).
...
def stitch_completion_after_windowed_vllm_generate(
    ...
    group_prompts: Sequence[RolloutPrompt],
    group_size: int,
    ...
    :type group_prompts: Sequence[RolloutPrompt]
...
def expert_lora_vllm_key_map(peft_model: nn.Module) -> dict[str, str]:
    ...
    # peft ParamWrapper is an optional extra this expert-key map needs.
...
def save_peft_adapter_for_vllm_rollout(
    ...
    *,
    ...
    # peft / safetensors: optional extras this vLLM adapter-export path needs.
