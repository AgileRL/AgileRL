...
def _transpose_hwc(
    observation: NumpyObsType | torch.Tensor,
    target: tuple[int, ...],
) -> NumpyObsType | torch.Tensor:
    """Permute a 3-D or 4-D HWC array or tensor onto channels-first *target*."""
...
def transpose_image_observation(
    ...
    if isinstance(original_space, spaces.Box) and len(original_space.shape) == 3:
        ...
        return _transpose_hwc(observation, target)
...
def get_obs_shape(space: spaces.Space) -> ObsShape:
    ...
    """
    if isinstance(space, LEAF_SPACE_TYPES):
