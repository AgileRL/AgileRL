...
def _wall_limit_reached(session: LLMRolloutSession) -> bool:
    ...
    if not _any_rank_flag(
        time.monotonic() >= session.wall_deadline, session.accelerator
    ):
        return False
...
def train_llm_rollout(
    ...
    session = _start_llm_rollout(pop, max_turns, env_factory, init_hp, run, accelerator)
    try:
