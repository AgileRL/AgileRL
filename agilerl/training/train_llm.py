...
def _save_elite_checkpoint(
    ...
    if save_elite and elite_path is not None:
        barrier()
        # All ranks participate: fitness histories are rank-identical and
        # save_llm_checkpoint synchronises internally (only rank 0 writes).
...
def finetune_llm_sft(
    ...
    for i in range(training_steps):
        ...
        for agent_idx, agent in enumerate(pop):
            ...
            next_prompts = training_env.step()
            agg_metrics = [
                safe_aggregate_metrics(loss),
                safe_aggregate_metrics(perplexity),
            ]
            if uses_env_fn:
                prompts_by_agent[agent_idx] = next_prompts
            else:
            ...
            if (i + 1) % evaluation_interval == 0:
                ...
                agg_test_metrics = [safe_aggregate_metrics(test_score)]
                barrier()
