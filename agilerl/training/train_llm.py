...
def _grpo_sampling_kwargs(
    ...
    *,
    ...
    """Return ``{"sampling_logps": ...}`` for the GRPO family, else ``{}``.

    The vLLM sampling-mismatch correction is a GRPO-family feature; the logps
    are threaded into ``learn()`` only when ``isinstance(agent, GRPO)`` and the
    caller-computed ``present`` flag confirms some were captured.
    """
    if isinstance(agent, GRPO) and present:
