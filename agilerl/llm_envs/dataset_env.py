...
def preference_collate_builder(
    ...
    tokenizer = inject_chat_template_kwargs(tokenizer, env.chat_template_kwargs)
...
def sft_collate_builder(
    ...
    def collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        eos = tokenizer.eos_token or ""
        completions = [response + eos for response in responses]
        if env.max_context_length is not None:
            pair_enc = tokenizer(
                prompts,
                completions,
            ...
            pair_ids = tokenizer(prompts, completions, truncation=True, padding=False)
            max_len = max(len(ids) for ids in pair_ids["input_ids"])
            pair_enc = tokenizer.pad(
                pair_ids,
                padding="max_length",
                max_length=max_len,
                return_tensors="pt",
