...
vLLM tests are spread across `vllm0..vllm{N-1}` xdist groups (see the `_N_PARALLEL_GROUPS` constant in `tests/conftest.py`) so up to N vLLM processes can initialise concurrently within a single pytest invocation. The Linux CI workflow inherits `-n auto` from `pyproject.toml`'s addopts and runs each Python-version matrix container on its own dedicated 16 GiB GPU (no sibling-container sharing). The group count is sized to comfortably exceed any current runner's `-n auto`, so concurrency is governed by CPU count (and implicitly by GPU memory pressure), not by group count. If a runner OOMs because too many heavy tests align across workers — measured peaks: ~4.4 GiB (`test_grpo_move_model_to_vllm`), ~3.0 GiB (`test_grpo_learn`), ~2.5 GiB (`test_grpo_clone_with_accelerator_vllm`); median per-test ~1.5 GiB — cap the vLLM phase by adding an explicit `-n N` to its workflow step (where N ≈ floor(16 GiB / 4.4 GiB) = 3 is the worst-case ceiling). **Either way, every `VLLMConfig` constructed in tests sets `kv_cache_memory_bytes`** (e.g. `32 * 1024 * 1024`), and that's what makes intra-container parallelism safe at all.
