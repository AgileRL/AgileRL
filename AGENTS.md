...
vLLM tests are split across `vllm0..vllm2` xdist groups (see `tests/conftest.py`) so several vLLM processes *can* initialise concurrently in the same pytest invocation when the GPU budget allows. The Linux CI workflow currently uses `-n 1` for the vLLM phase because all 4 matrix containers share a single GPU (~1/4 of VRAM each), so the groups are effectively inert there; they become active when you run vLLM tests in a single-matrix or wider-GPU configuration. **Either way, every `VLLMConfig` constructed in tests sets `kv_cache_memory_bytes`** (e.g. `32 * 1024 * 1024`), and that's what makes intra-container parallelism safe at all.
