...
Runs SFT with default hyperparameters from ``configs/training/sft.yaml``.
...
if __name__ == "__main__":
    resolved = benchmark_cli_llm.parse_offline_llm_cli(
        ...
        description="Supervised Fine-Tuning (SFT) benchmarking.",
    )
    save_path = resolved.args.save_path or (
        f"outputs/{datetime.now().strftime('%Y%m%d_%H%M%S')}_SFT"
    )
    main(
