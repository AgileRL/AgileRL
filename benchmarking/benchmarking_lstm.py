...
def my_process_data(env: StocksEnv):
    start = env.frame_bound[0] - env.window_size
    end = env.frame_bound[1]
    prices = env.df.loc[:, "Low"].to_numpy()[start:end]
    signal_features = env.df.loc[:, ["Close", "Open", "High", "Low"]].to_numpy()[
        start:end
    ]
...
def main_off_policy(INIT_HP, MUTATION_PARAMS, NET_CONFIG):
    # Set device
    ...
    def make_env():
        return MyStocksEnv(**kwargs)

    env = make_vect_envs(make_env, num_envs=INIT_HP["NUM_ENVS"])
...
def main_on_policy(INIT_HP, MUTATION_PARAMS, NET_CONFIG):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("============ AgileRL ============")
    print(f"DEVICE: {device}")
...
if __name__ == "__main__":
    off_policy = False
    if off_policy:
        with open("configs/training/dqn/dqn_lstm.yaml") as file:
            config = yaml.safe_load(file)
        INIT_HP = config["INIT_HP"]
        MUTATION_PARAMS = config["MUTATION_PARAMS"]
        NET_CONFIG = config["NET_CONFIG"]
        main_off_policy(INIT_HP, MUTATION_PARAMS, NET_CONFIG)
    else:
        with open("configs/training/ppo/ppo_lstm.yaml") as file:
            config = yaml.safe_load(file)
        INIT_HP = config["INIT_HP"]
        MUTATION_PARAMS = config["MUTATION_PARAMS"]
        NET_CONFIG = config["NET_CONFIG"]
        main_on_policy(INIT_HP, MUTATION_PARAMS, NET_CONFIG)
