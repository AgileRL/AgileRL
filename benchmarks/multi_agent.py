"""Multi-agent benchmarking script.

Loads a training manifest for a multi-agent algorithm (MADDPG, MATD3,
IPPO, etc.) and runs evolutionary training via
``LocalTrainer.from_manifest()``.

Example usage::

    python benchmarks/multi_agent.py --manifest configs/training/multi_agent/maddpg.yaml
    python benchmarks/multi_agent.py --manifest configs/training/multi_agent/ippo.yaml --device cuda
    python benchmarks/multi_agent.py --manifest configs/training/multi_agent/matd3.yaml --wb
"""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for multi-agent training."""
    parser = argparse.ArgumentParser(
        description="Run multi-agent evolutionary RL training from a manifest.",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default="configs/training/multi_agent/ippo.yaml",
...
def main() -> None:
    """Run multi-agent evolutionary RL training from a manifest."""
    args = parse_args()
