...
def _load_py(name: str, path: Path) -> object:
...
def _materialize_digit_prior_model(
    ...
    """
    # transformers/torch only when materialising a digit-prior snapshot
    import torch
...
def main() -> int:
    ...
    args = parse_args()
    # LLM stack after argv parse so --help works without those deps.
    import torch

    from agilerl.training.llm import train_llm_rollout
    from agilerl.distributed import get_rank, is_main_process
    from agilerl.utils.utils import create_population

    repo = Path(__file__).resolve().parents[2]
    debug_dir = repo / "demos/llm/debugging"
    llm_debug_utils = _load_py("llm_debug_utils", debug_dir / "llm_debug_utils.py")
    tiny_model = _load_py("tiny_model", debug_dir / "tiny_model.py")
    lora_config_from_dict = getattr(llm_debug_utils, "lora_config_from_dict")
    TinyDigitTokenizer = getattr(tiny_model, "TinyDigitTokenizer")
    build_tiny_actor_network = getattr(tiny_model, "build_tiny_actor_network")
    ...
    vllm_config = None
    if args.use_vllm:
        # VLLMConfig is part of the optional LLM stack deferred until after --help.
    ...
    if args.model_name:
        # transformers is part of the optional LLM stack deferred until after --help.
        ...
        # Debug YAML lists GPT2 target_modules (c_attn/c_proj/c_fc) which
        # Qwen2/Qwen3 do not have; use attention projections.
        ...
        if is_moe:
            # Packed-expert LoRA needs dropout 0.
            lora_dict.setdefault("lora_dropout", 0.0)
            if lora_dict.get("target_parameters") is None:
                # Prefer attention LoRA for ConstantTarget learning.
    ...
    fsdp_cfg = None
    if args.fsdp:
        # FSDPConfig is part of the optional LLM stack deferred until after --help.
