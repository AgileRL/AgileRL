...
- :class:`JsonlLogger`        — writes per-step metrics to JSONL
...
class EnvManifest:
    ...
    def capture(self, repo: Path, fsdp_config: dict | None = None) -> "EnvManifest":
        ...
        try:
            # torch is optional so EnvManifest works on hosts without it.
            import torch
...
class TeeStream:
    ...
    def __init__(
        self, original: Any, file: io.TextIOBase, lock: threading.Lock
    ) -> None:
...
class StdoutTee:
    ...
    def __enter__(self) -> "StdoutTee":
        ...
        sys.stdout = TeeStream(self._old_stdout, self._stdout_file, self._lock)
        sys.stderr = TeeStream(self._old_stderr, self._stderr_file, self._lock)
        return self
...
class JsonlLogger:
    """Logger that appends ``MetricsReport.to_dict()`` to a JSONL file.
...
BOX_RE = re.compile(r"[\u2500-\u257f]")  # Rich table box-drawing chars
FLOAT_RE = r"([-+]?\d+\.?\d*(?:[eE][-+]?\d+)?|nan|inf|-inf)"
LOSS_PATTERNS = [
    re.compile(r"(?:train/)?loss\s*" + FLOAT_RE, re.IGNORECASE),
    re.compile(r"loss:\s*" + FLOAT_RE, re.IGNORECASE),
    re.compile(r"Loss\s+" + FLOAT_RE, re.IGNORECASE),
]
EVAL_PATTERNS = [
    re.compile(r"(?:eval|accuracy|hit.?rate)\s*" + FLOAT_RE, re.IGNORECASE),
    re.compile(r"eval:\s*" + FLOAT_RE, re.IGNORECASE),
]
STEP_PATTERNS = [
...
class LossCurveParser:
    ...
    def feed_line(self, line: str) -> None:
        ...
        clean = BOX_RE.sub(" ", clean).strip()
        if not clean:
            return

        step = self._extract_step(clean)
        loss = self._extract_float(clean, LOSS_PATTERNS)
        eval_m = self._extract_float(clean, EVAL_PATTERNS)
        ...
        if step is not None or loss is not None:
            ...
            self._writer.record(StepRecord(step=s, loss=loss, eval_metric=eval_m))
...
class RankHealthMonitor:
    ...
    def check(self, step: int, loss: float | None) -> dict[str, Any]:
        ...
        try:
            # torch.distributed is optional when world_size == 1 / CPU-only.
            import torch
